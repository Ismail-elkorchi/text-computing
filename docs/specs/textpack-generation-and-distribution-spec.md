...
| `task-supported` | At least one generated language, domain, corpus, KB, parallel, or historical pack targets the language. |
...
This API is a coverage declaration, not a language-quality claim. `task-supported` MUST reflect actual generated pack targets and known gaps; it MUST NOT imply feature-complete support.
...
A fully supported language MUST cover the Text Computing feature surface through generated packs, external artifacts, or documented unavailable capabilities:
...
| `kb` | entity labels, aliases, sense links, semantic relations, ontologies, thesauri, or KB mappings |
| `search` | analyzer profiles, normalization/stemming hooks, ranking fields, and index configuration |
| `corpus` | redistributable corpora, samples, derived statistics, evaluation records, or artifact descriptors |
| `quality` | text-quality, OCR/ATR, annotation-quality, corpus-quality, readability, and style profiles |
| `parallel` | bilingual lexicons, translation memories, alignments, parallel samples, or artifact descriptors |
...
npm is the distribution mechanism for installable textpack modules:
...
npm MUST NOT be used to silently deliver huge corpora, full indexes, massive snapshots, or heavy model/resource archives through installation side effects.
