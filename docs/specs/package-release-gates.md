...
| `@ismail-elkorchi/textpipeline` | `production-candidate` | Snapshot-backed cache import/export, caller-provided worker execution, deterministic worker-pool assignment reports, and caller-managed recovery plans exist; remote orchestration, distributed scheduling, and automatic retry execution are not release-ready. | Does not block alpha; it limits production-candidate operational statements beyond deterministic local execution, caller-managed cache snapshot semantics, caller-provided worker adapters, deterministic local worker-pool assignment, and recovery-plan metadata. | packages/textpipeline/src/index.ts<br>packages/textpipeline/test<br>examples/textpipeline-cache-snapshot-consumer.mjs<br>examples/textpipeline-worker-batch-consumer.mjs<br>examples/textpipeline-worker-pool-consumer.mjs<br>examples/textpipeline-recovery-plan-consumer.mjs<br>schemas/textpipeline-worker-run-report-v1.schema.json<br>schemas/textpipeline-worker-pool-run-report-v1.schema.json<br>schemas/textpipeline-recovery-plan-v1.schema.json |
