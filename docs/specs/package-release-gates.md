...
| --- | --- | --- | --- | --- |
| `@ismail-elkorchi/textconformance` | `beta` | Benchmark runner records exist for local suite execution; stable calibrated threshold policy remains outside the alpha scope. | Does not block alpha because alpha requires declared conformance reports, separated benchmark records, and checks, not calibrated benchmark thresholds. | packages/textconformance/src/index.ts<br>schemas/textconformance-benchmark-report-v1.schema.json<br>packages/textconformance/test<br>examples/textconformance-benchmark-runner-consumer.mjs |
| `@ismail-elkorchi/textcorpus` | `beta` | Corpus analysis, retrieval, and corpus scoring remain frozen-corpus only without external relevance datasets or broad field weighting. | Blocks broader corpus and retrieval release statements; it does not authorize task expansion during alpha foundation release. | packages/textcorpus/src/index.ts<br>packages/textcorpus/test<br>examples/textcorpus-retrieval-index-storage-consumer.mjs |
...
| `@ismail-elkorchi/textpipeline` | `production-candidate` | Snapshot-backed cache import/export exists for caller-managed persistence; remote orchestration, distributed scheduling, worker execution, and automatic recovery coordination are not release-ready. | Does not block alpha; it limits production-candidate operational statements beyond deterministic local execution and caller-managed cache snapshot semantics. | packages/textpipeline/src/index.ts<br>packages/textpipeline/test<br>examples/textpipeline-cache-snapshot-consumer.mjs |
