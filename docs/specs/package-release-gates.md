...
| `@ismail-elkorchi/textpipeline` | `production-candidate` | Snapshot-backed cache import/export, caller-provided worker execution, and deterministic worker-pool assignment reports exist; remote orchestration, distributed scheduling, and automatic recovery coordination are not release-ready. | Does not block alpha; it limits production-candidate operational statements beyond deterministic local execution, caller-managed cache snapshot semantics, caller-provided worker adapters, and deterministic local worker-pool assignment. | packages/textpipeline/src/index.ts<br>packages/textpipeline/test<br>examples/textpipeline-cache-snapshot-consumer.mjs<br>examples/textpipeline-worker-batch-consumer.mjs<br>examples/textpipeline-worker-pool-consumer.mjs<br>schemas/textpipeline-worker-run-report-v1.schema.json<br>schemas/textpipeline-worker-pool-run-report-v1.schema.json |
