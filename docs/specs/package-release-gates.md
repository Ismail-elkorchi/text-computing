...
| `@ismail-elkorchi/textlab` | `beta` | Standalone external-tool execution reports and deterministic inspection-session API and CLI navigation exist; terminal interactive views remain outside alpha scope, and corpus artifact browsing is bounded to deterministic API and CLI pagination. | Blocks broader inspection-tooling statements; it does not block foundation alpha packages unless their smoke output cannot be inspected. | packages/textlab/src/index.ts<br>packages/textlab/test/index.test.mjs<br>examples/textlab-corpus-artifact-consumer.mjs<br>examples/textlab-external-tool-consumer.mjs<br>examples/textlab-inspection-session-consumer.mjs<br>examples/textlab-inspection-session-cli-consumer.mjs<br>schemas/textlab-external-tool-execution-report-v1.schema.json<br>schemas/textlab-inspection-session-v1.schema.json |
