...
| --- | --- | --- | --- | --- |
| `@ismail-elkorchi/textconformance` | `beta` | Benchmark threshold policy evaluation, cross-host calibration reports, and benchmark matrix reports exist for deterministic benchmark reports over caller-provided host reports; host provisioning remains outside the alpha scope. | Does not block alpha because alpha requires declared conformance reports, separated benchmark records, local threshold evaluation, caller-provided calibration and matrix reports, and package checks, not host provisioning. | packages/textconformance/src/index.ts<br>schemas/textconformance-benchmark-report-v1.schema.json<br>schemas/textconformance-benchmark-calibration-report-v1.schema.json<br>schemas/textconformance-benchmark-matrix-report-v1.schema.json<br>schemas/textconformance-benchmark-threshold-policy-v1.schema.json<br>schemas/textconformance-benchmark-threshold-evaluation-v1.schema.json<br>packages/textconformance/test<br>examples/textconformance-benchmark-runner-consumer.mjs<br>examples/textconformance-benchmark-calibration-consumer.mjs<br>examples/textconformance-benchmark-matrix-consumer.mjs |
...
| `@ismail-elkorchi/textlab` | `beta` | Standalone external-tool execution reports, textconformance benchmark matrix inspection, and deterministic inspection-session API and CLI navigation exist; terminal interactive views remain outside alpha scope, and corpus artifact browsing is bounded to deterministic API and CLI pagination. | Blocks broader inspection-tooling statements; it does not block foundation alpha packages unless their smoke output cannot be inspected. | packages/textlab/src/index.ts<br>packages/textlab/test/index.test.mjs<br>examples/textlab-benchmark-matrix-consumer.mjs<br>examples/textlab-corpus-artifact-consumer.mjs<br>examples/textlab-external-tool-consumer.mjs<br>examples/textlab-inspection-session-consumer.mjs<br>examples/textlab-inspection-session-cli-consumer.mjs<br>schemas/textlab-external-tool-execution-report-v1.schema.json<br>schemas/textlab-inspection-session-v1.schema.json |
