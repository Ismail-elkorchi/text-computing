...
Issue `#14` does not permit corpus scoring behavior until formulas, token policy, numeric tolerance, and expected-output format are frozen.
...
`npm run -s check:fixtures` validates the corpus slice manifest, formula manifest, expected outputs, required readiness documentation headings, and fixture boundary before any feature PR for issue `#14`.
