...
| Dependency parsing and CoNLL-U/UD interchange | `slice-proven` for CoNLL-U I/O; `readiness-only` for parser arcs | CoNLL-U/UD research ledger, valid/invalid CoNLL-U fixtures, expected round-trip outputs, dependency-target contract, parser-readiness expected arcs, and one executed spaCy parser capture | Needs broader executed parser comparator captures, broader UD slices, and dependency parser implementation. |
