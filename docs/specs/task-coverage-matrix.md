...
| Relation extraction | `readiness-only` | Typed relation schemas, readiness fixtures, negative controls, and conformance report | Needs recorded expected outputs, executed comparator captures, feature behavior, and corpus evaluation. |
| Coreference | `readiness-only` | Mention/chain schemas, readiness fixtures, negative controls, and conformance report | Needs recorded expected outputs, executed comparator captures, feature behavior, and corpus evaluation. |
...
| Full retrieval | `slice-proven` | Explicit-token retrieval fixture, query parser, inverted-index lookup, BM25 ranking, snippets, explain output, and conformance report | Needs relevance judgments, larger corpora, fielded query syntax, streaming index behavior, and scale/performance budgets. |
