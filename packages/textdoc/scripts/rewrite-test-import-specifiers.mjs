...
const distRootDir = path.join(workspaceRoot, "dist-test");
...
async function listRewriteTargetPathsRecursive(dirPath) {
	...
	for (const entry of entries) {
		...
		if (entry.isDirectory()) {
			rewriteTargetPaths.push(
				...(await listRewriteTargetPathsRecursive(entryPath)),
			);
...
async function main() {
	try {
		const rewriteTargetPaths =
			await listRewriteTargetPathsRecursive(distRootDir);
		await Promise.all(
