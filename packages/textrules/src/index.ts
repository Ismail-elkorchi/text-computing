...
export type TextRulesRelationExtractionRevision =
	typeof relationExtractionRevision;
...
export type TextRulesCoreferenceMentionKind =
	| "proper"
	| "nominal"
	| "pronoun"
	| "singleton";
...
interface TextRulesSourceRef {
	readonly id: string;
	readonly sha256?: string;
}

interface TextRulesReferenceRef {
	readonly kind: string;
	readonly id: string;
}

interface TextRulesProvenance {
	readonly source?: TextRulesSourceRef;
	readonly references?: readonly TextRulesReferenceRef[];
}

interface TextRulesConfidence {
	readonly value: number;
	readonly method?: string;
}

interface TextRulesSpanTarget {
	readonly kind: "span";
...
interface TextRulesWorkingDocumentTarget {
	readonly kind: "document";
}

interface TextRulesAnnotationTarget {
	readonly kind: "annotation";
	readonly annotationId: string;
}

type TextRulesTarget =
	| TextRulesSpanTarget
	| TextRulesWorkingDocumentTarget
	| TextRulesAnnotationTarget;

interface TextRulesAnnotation {
	...
	readonly targets: readonly TextRulesTarget[];
	readonly notes?: readonly string[];
	readonly provenance?: TextRulesProvenance;
	readonly confidence?: TextRulesConfidence;
}

interface TextRulesStringAlternative {
	readonly id: string;
...
interface TextRulesMorphologyAlternative {
	readonly id: string;
	readonly rank: number;
	readonly features: readonly TextRulesFeature[];
	readonly notes?: readonly string[];
}

interface TextRulesTokenAnnotation extends TextRulesAnnotation {
	readonly kind: "token";
	readonly tokenKind: string;
	readonly text?: string;
}

interface TextRulesSentenceAnnotation extends TextRulesAnnotation {
	readonly kind: "sentence";
	readonly sentenceKind: string;
	readonly text?: string;
}

interface TextRulesPosAnnotation extends TextRulesAnnotation {
	readonly kind: "pos";
	readonly tagSet: string;
	readonly alternatives: readonly TextRulesStringAlternative[];
}

interface TextRulesLemmaAnnotation extends TextRulesAnnotation {
	readonly kind: "lemma";
	readonly alternatives: readonly TextRulesStringAlternative[];
}

interface TextRulesMorphologyAnnotation extends TextRulesAnnotation {
	readonly kind: "morphology";
	readonly alternatives: readonly TextRulesMorphologyAlternative[];
}

interface TextRulesEntityAnnotation extends TextRulesAnnotation {
...
interface TextRulesRelationAnnotation extends TextRulesAnnotation {
...
interface TextRulesDependencyNodeAnnotation extends TextRulesAnnotation {
...
interface TextRulesDependencyAnnotation extends TextRulesAnnotation {
...
interface TextRulesCoreferenceMentionAnnotation extends TextRulesAnnotation {
	readonly kind: "coreference-mention";
	readonly mentionType: string;
	readonly text?: string;
}

interface TextRulesCoreferenceChainAnnotation extends TextRulesAnnotation {
	readonly kind: "coreference-chain";
	readonly mentionIds: readonly string[];
	readonly representativeMentionId?: string;
}

interface TextRulesExtensionAnnotation extends TextRulesAnnotation {
	readonly kind: "extension";
	readonly extensionId: string;
	readonly extensionSchema?: { readonly [key: string]: unknown };
	readonly payload?: { readonly [key: string]: unknown };
}

interface TextRulesLayer<
	TAnnotation extends TextRulesAnnotation = TextRulesAnnotation,
> {
	readonly [key: string]: unknown;
	readonly id: string;
	readonly kind: string;
...
interface TextRulesSpanMap {
	...
	readonly segments: readonly {
		readonly source: { readonly startCU: number; readonly endCU: number };
		readonly target: { readonly startCU: number; readonly endCU: number };
		readonly kind: string;
		readonly reversible?: boolean;
	}[];
}

interface TextRulesWorkingDocument {
	readonly [key: string]: unknown;
	...
	readonly text?: string;
	readonly source?: TextRulesSourceRef;
	readonly units: { readonly text: "utf16-code-unit" };
	readonly views: readonly TextRulesView[];
	readonly spanMaps: readonly TextRulesSpanMap[];
	readonly layers: readonly TextRulesLayer[];
}
...
function firstRecordValue<T>(
	record: Readonly<Record<string, T>>,
): T | undefined {
	const firstKey = Object.keys(record).sort((left, right) =>
		left.localeCompare(right),
	)[0];
	return firstKey === undefined ? undefined : record[firstKey];
}

function textRulesDocumentRevision(document: TextDocument): string {
	const revision = document.metadata["revision"];
	return typeof revision === "string" && revision.length > 0 ? revision : "0";
}

function textRulesViewKind(kind: string): TextView["kind"] {
	if (
		kind === "raw" ||
		kind === "decoded" ||
		kind === "normalized" ||
		...
		kind === "transliterated" ||
		kind === "transcribed" ||
		kind === "historical-normalized" ||
		kind === "ocr-corrected" ||
		kind === "noisy-normalized" ||
		kind === "search" ||
		kind === "task" ||
		kind === "external"
	) {
...
function textRulesLayerKind(type: string): string {
	const dotIndex = type.indexOf(".");
	return dotIndex < 0 ? type : type.slice(0, dotIndex);
}

function finalLayerTypeFromTextRulesKind(kind: string): string {
	if (kind === "token") return "token.word";
	if (kind === "sentence") return "sentence";
	return kind;
}

function textRulesEvidence(
	annotation: TextRulesAnnotation,
	inputViewIds: readonly string[],
): Evidence {
	const references = annotation.provenance?.references ?? [];
	const resourceIds = references
		.filter((reference) => reference.kind !== "textrules-rule")
		.map((reference) => reference.id);
	const ruleIds = references
		.filter((reference) => reference.kind === "textrules-rule")
		.map((reference) => reference.id);
	return {
		mode: "rule",
		exactness: "E1",
		producer: "@ismail-elkorchi/textrules",
		packageName: "@ismail-elkorchi/textrules",
		packageVersion: "0.1.0",
		...(resourceIds.length > 0 ? { resourceIds } : {}),
		...(ruleIds.length > 0 ? { ruleIds } : {}),
		inputViewIds: inputViewIds.length > 0 ? inputViewIds : ["analysis-view"],
	};
}

function textRulesAnnotationValue(
	annotation: TextRulesAnnotation,
	...
	if (lifecycle.state !== "active") result.lifecycle = lifecycle;
	return Object.keys(result).length === 0 ? undefined : result;
}

function finalAnnotationFromTextRules(
	layerId: string,
	layerType: string,
	annotation: TextRulesAnnotation,
): Annotation<Readonly<Record<string, unknown>>> {
	const spans = annotation.targets
		.filter((target): target is TextRulesSpanTarget => target.kind === "span")
		.map((target) => ({
			viewId: target.viewId,
			span: {
				start: target.startCU,
				end: target.endCU,
				unit: "utf16-code-unit" as const,
			},
		}));
	const value = textRulesAnnotationValue(annotation);
	return {
		id: annotation.id,
		layer: layerId,
		type: annotation.kind === "token" ? layerType : annotation.kind,
		spans,
		...(value === undefined ? {} : { value }),
		evidence: textRulesEvidence(
			annotation,
...
function textRulesAnnotationFromFinal(
	annotation: Annotation,
): TextRulesAnnotation {
	const value =
		annotation.value !== null && typeof annotation.value === "object"
			? (annotation.value as Record<string, unknown>)
			: {};
	const kind = textRulesLayerKind(annotation.type);
	const targets: TextRulesTarget[] = annotation.spans.map((span) => ({
		kind: "span",
		viewId: span.viewId,
	...
	return {
		...
		targets,
		...value,
		...(kind === "token" && typeof value.text === "string"
			? { tokenKind: "lexical-token", text: value.text }
			: {}),
		...(kind === "sentence" && typeof value.text === "string"
			? { sentenceKind: "uax29-sentence", text: value.text }
			: {}),
	} as TextRulesAnnotation;
}

function finalLayerFromTextRules(
	layer: TextRulesLayer,
): AnnotationLayer<Readonly<Record<string, unknown>>> {
	const layerType = finalLayerTypeFromTextRulesKind(layer.kind);
	const annotations = recordById(
		layer.annotations.map((annotation) =>
			finalAnnotationFromTextRules(layer.id, layerType, annotation),
		),
	);
	return {
		id: layer.id,
		type: layerType,
		...(layer.viewId === undefined ? {} : { viewId: layer.viewId }),
		annotations,
	};
}

function textRulesLayerFromFinal(layer: AnnotationLayer): TextRulesLayer {
	return {
		id: layer.id,
		kind: textRulesLayerKind(layer.type),
		...(layer.viewId === undefined ? {} : { viewId: layer.viewId }),
		annotations: Object.values(layer.annotations).map((annotation) =>
			textRulesAnnotationFromFinal(annotation),
		),
	};
}

function finalSpanMapFromTextRules(spanMap: TextRulesSpanMap): SpanMap {
	return {
		id: spanMap.id,
		sourceViewId: spanMap.sourceViewId,
		targetViewId: spanMap.targetViewId,
		entries: spanMap.segments.map((segment) => ({
			source: {
				start: segment.source.startCU,
				end: segment.source.endCU,
				unit: "utf16-code-unit",
			},
			target: {
				start: segment.target.startCU,
				end: segment.target.endCU,
				unit: "utf16-code-unit",
			},
			relation: segment.kind === "unchanged" ? "identity" : "aligned",
		})),
	};
}

function textRulesSpanMapFromFinal(spanMap: SpanMap): TextRulesSpanMap {
	return {
		id: spanMap.id,
		sourceViewId: spanMap.sourceViewId,
		targetViewId: spanMap.targetViewId,
		lifecycle: { state: "active" },
		segments: spanMap.entries.map((entry) => ({
			source: { startCU: entry.source.start, endCU: entry.source.end },
			target: { startCU: entry.target.start, endCU: entry.target.end },
			kind: entry.relation === "identity" ? "unchanged" : entry.relation,
			reversible: entry.relation === "identity",
		})),
	};
}

interface FinalDocumentFromTextRulesOptions {
	readonly base?: TextDocument;
	readonly replaceViewIds?: readonly string[];
	readonly replaceSpanMapIds?: readonly string[];
	readonly replaceLayerIds?: readonly string[];
}

function mergeGeneratedRecord<T extends { readonly id: string }>(
	...
	for (const [id, value] of Object.entries(generated).sort(([left], [right]) =>
		left.localeCompare(right),
	)) {
		if (base[id] === undefined || replace.has(id)) {
			merged[id] = value;
		}
	}
	return recordById(Object.values(merged));
}

function finalDocumentFromTextRules(
	document: TextRulesWorkingDocument,
	options: FinalDocumentFromTextRulesOptions = {},
): TextDocument {
	const text = document.text ?? "";
	const sourceId = document.source?.id ?? document.documentId;
	const source: TextSource = {
		id: sourceId,
		text,
		inputKind: "string",
		wellFormed: true,
		...(document.source?.sha256 === undefined
			? {}
			: { metadata: { sha256: document.source.sha256 } }),
	};
	const views = recordById(
		document.views.map(
			(view): TextView => ({
				id: view.id,
				kind: textRulesViewKind(view.kind),
				text: view.text ?? text,
				...(view.parentViewId === undefined
					? {}
					: { sourceViewId: view.parentViewId }),
				...(view.spanMapIds?.[0] === undefined
					? {}
					: { spanMapId: view.spanMapIds[0] }),
				transform: {
					kind: view.kind,
					producer: "@ismail-elkorchi/textrules",
					...(view.parentViewId === undefined
	...
	const generated: TextDocument = {
		id: document.documentId,
		sources: { [sourceId]: source },
		views,
		spanMaps: recordById(
			document.spanMaps.map((spanMap) => finalSpanMapFromTextRules(spanMap)),
		),
		layers: recordById(
			document.layers.map((layer) => finalLayerFromTextRules(layer)),
		),
		graphs: {},
		metadata: {
			revision: document.revision,
			schemaVersion: document.schemaVersion,
			...(document.unicodeVersion === undefined
				? {}
				: { unicodeVersion: document.unicodeVersion }),
			...(document.notes === undefined ? {} : { notes: document.notes }),
		},
	};
	if (options.base === undefined) return generated;
	return {
		...generated,
		sources:
			Object.keys(options.base.sources).length > 0
				? options.base.sources
				: generated.sources,
		views: mergeGeneratedRecord(
			options.base.views,
			generated.views,
			options.replaceViewIds,
		),
		spanMaps: mergeGeneratedRecord(
			options.base.spanMaps,
			generated.spanMaps,
			options.replaceSpanMapIds,
		),
		layers: mergeGeneratedRecord(
			options.base.layers,
			generated.layers,
			options.replaceLayerIds,
		),
		graphs: options.base.graphs,
		metadata: {
...
function textRulesWorkingDocumentFromFinal(
	document: TextDocument,
): TextRulesWorkingDocument {
	const source = firstRecordValue(document.sources);
	const firstView = firstRecordValue(document.views);
	const text = firstView?.text ?? source?.text ?? "";
	return {
		schemaVersion: documentSchemaVersion,
		documentId: document.id,
		revision: textRulesDocumentRevision(document),
		textLengthCU: text.length,
		text,
		...(source === undefined ? {} : { source: { id: source.id } }),
		units: { text: "utf16-code-unit" },
		views: Object.values(document.views).map((view) => ({
			id: view.id,
			kind: view.kind,
			text: view.text,
			...(view.sourceViewId === undefined
				? {}
				: { parentViewId: view.sourceViewId }),
			...(view.spanMapId === undefined ? {} : { spanMapIds: [view.spanMapId] }),
		})),
		spanMaps: Object.values(document.spanMaps).map((spanMap) =>
			textRulesSpanMapFromFinal(spanMap),
		),
		layers: Object.values(document.layers).map((layer) =>
			textRulesLayerFromFinal(layer),
		),
	};
}
...
export interface TextRulesLexiconAnalysis {
	...
	readonly morphology?: readonly TextRulesFeature[];
	readonly notes?: readonly string[];
}
...
export interface TextRulesTextDocTokenPatternInput
	extends TextRulesTextDocTokenLayerOptions {
	readonly document: TextDocument;
	readonly pattern: TextRulesTokenPattern;
}

export interface TextRulesTextDocTokenPatternsInput
	extends TextRulesTextDocTokenLayerOptions {
	readonly document: TextDocument;
...
export type TextRulesConflictPolicy =
	| "emit-all"
	| "first-win"
	| "longest-win"
	| "error";

export interface TextRulesRuleReadsV1
	extends TextRulesTextDocTokenLayerOptions {
	readonly viewId?: string;
	readonly layerId?: string;
	readonly annotationKind?: TextRulesAnnotation["kind"];
}
...
export interface TextRulesRuleWhenV1 {
	...
	readonly annotationKind?: TextRulesAnnotation["kind"];
	readonly feature?: TextRulesFeature;
}
...
export interface TextRulesRunResult {
	readonly document: TextDocument;
	readonly annotations: readonly TextRulesExtensionAnnotation[];
	readonly diagnostics: readonly TextRulesDiagnostic[];
	readonly rewrites: readonly TextRulesRewriteArtifact[];
}

export type TextRulesTextPackRuleKind =
	| "stopword"
	| "lexicon"
	| "gazetteer"
	| "rule-list";
...
export interface TextRulesTextPackRunInput
	extends TextRulesTextDocTokenLayerOptions {
	readonly document: TextDocument;
	readonly compiled: TextRulesCompiledTextPackRules;
...
export interface TextRulesTextPackPipelineProcessor {
	...
	run(document: TextDocument): TextRulesTextPackPipelineProcessorRunResult;
}

export interface TextRulesTextPackPipelineProcessorOptions
	extends TextRulesTextDocTokenLayerOptions {
	readonly id?: string;
	readonly version?: string;
	readonly dependsOn?: readonly string[];
...
interface TextRulesResolvedAnalysis extends TextRulesLexiconAnalysis {
	readonly resourceRefs: readonly TextRulesReferenceRef[];
}

type TextRulesPosLayer = TextRulesLayer<TextRulesPosAnnotation>;
type TextRulesLemmaLayer = TextRulesLayer<TextRulesLemmaAnnotation>;
type TextRulesMorphologyLayer = TextRulesLayer<TextRulesMorphologyAnnotation>;
type TextRulesEntityLayer = TextRulesLayer<TextRulesEntityAnnotation>;
type TextRulesRelationLayer = TextRulesLayer<TextRulesRelationAnnotation>;
type TextRulesDependencyNodeLayer =
	TextRulesLayer<TextRulesDependencyNodeAnnotation>;
type TextRulesDependencyLayer = TextRulesLayer<TextRulesDependencyAnnotation>;
type TextRulesCoreferenceMentionLayer =
	TextRulesLayer<TextRulesCoreferenceMentionAnnotation>;
type TextRulesCoreferenceChainLayer =
	TextRulesLayer<TextRulesCoreferenceChainAnnotation>;
...
export interface TextRulesRelationArgumentTemplate
	extends TextRulesCaptureSpanTemplate {
	readonly role: string;
}

export interface TextRulesRelationEvidenceTemplate
	extends TextRulesCaptureSpanTemplate {}
...
export interface TextRulesCoreferenceMentionTemplate
	extends TextRulesCaptureSpanTemplate {
	readonly id: string;
	readonly kind: TextRulesCoreferenceMentionKind;
	readonly notes?: readonly string[];
...
function compareFeatures(
	left: TextRulesFeature,
	right: TextRulesFeature,
): number {
	return (
...
function normalizeFeatures(
	features: readonly TextRulesFeature[],
): readonly TextRulesFeature[] {
	return [...features].sort(compareFeatures);
}

function stableMorphKey(features: readonly TextRulesFeature[]): string {
...
function uniqueReferences(
	references: readonly TextRulesReferenceRef[],
): readonly TextRulesReferenceRef[] {
	const seen = new Set<string>();
	const result: TextRulesReferenceRef[] = [];
...
): readonly TextRulesStringAlternative[] {
...
): readonly TextRulesMorphologyAlternative[] {
	return values.map((value, index) => ({
...
): readonly TextRulesTokenAnnotation[] {
...
function createSentenceAnnotations(
	...
	sourceId: string,
	sourceSha256: string | undefined,
): readonly TextRulesSentenceAnnotation[] {
...
export function tokenizeTextRulesText(
	text: string,
): readonly TextRulesTokenSpan[] {
	return tokenizeTextForRules(text);
}

export function tokenizeTextRulesFixtureText(
	text: string,
): readonly TextRulesTokenSpan[] {
	return tokenizeTextForRules(text);
}
...
function tokenLayerFromTextDoc(
...
): TextRulesLayer<TextRulesTokenAnnotation> {
	...
	return layer as TextRulesLayer<TextRulesTokenAnnotation>;
}

function spanTargetForTextRulesToken(annotation: TextRulesTokenAnnotation): {
	readonly viewId: string;
	readonly startCU: number;
	readonly endCU: number;
} {
...
function tokenTextFromTextRulesAnnotation(
	document: TextRulesWorkingDocument,
	annotation: TextRulesTokenAnnotation,
	startCU: number,
	endCU: number,
): string {
	...
	if (document.text === undefined) {
		throw new TypeError(
			`token annotation ${annotation.id} requires text or document.text`,
		);
	}
	return document.text.slice(startCU, endCU);
}

function textRulesTokenSpansFromWorkingDocument(
	document: TextRulesWorkingDocument,
	options: TextRulesTextDocTokenLayerOptions = {},
): readonly TextRulesTokenSpan[] {
	return tokenLayerFromTextDoc(document, options)
		.annotations.map((annotation) => {
			...
			return {
				...
				endCU: span.endCU,
				viewId: span.viewId,
				text: tokenTextFromTextRulesAnnotation(
					document,
					annotation,
					span.startCU,
					span.endCU,
...
export function textRulesTokenSpansFromTextDoc(
	...
	return textRulesTokenSpansFromWorkingDocument(
		textRulesWorkingDocumentFromFinal(document),
		options,
	);
}
...
export function matchTextDocTokenPattern(
	...
	return matchTextRulesTokenPattern(
		textRulesTokenSpansFromTextDoc(
			input.document,
			input.tokenLayerId === undefined
				? {}
				: { tokenLayerId: input.tokenLayerId },
		),
		input.pattern,
	);
}
...
function patternAtomMatches(
	...
	return atom.values.some(
		(value) => tokenText === normalizePatternValue(value, caseSensitive),
	);
}

function comparePatternMatches(
	left: TextRulesPatternMatch,
	right: TextRulesPatternMatch,
): number {
...
export function matchTextRulesTokenPattern(
	...
	for (
		let startIndex = 0;
		startIndex <= tokens.length - pattern.atoms.length;
		startIndex += 1
	) {
		...
		for (let atomIndex = 0; atomIndex < pattern.atoms.length; atomIndex += 1) {
			...
			if (
				atom === undefined ||
				token === undefined ||
				!patternAtomMatches(atom, token, caseSensitive)
			) {
				matched = false;
				break;
			}
...
export function rewriteTextRulesTokenTexts(
	...
	const orderedMatches = matchesByRule.sort(
		(left, right) =>
			comparePatternMatches(left.match, right.match) ||
	...
	while (cursor < tokens.length) {
		const selected = orderedMatches.find(
			(entry) => entry.match.startTokenIndex === cursor,
		);
...
function isTextRulesConflictPolicy(
	value: unknown,
): value is TextRulesConflictPolicy {
	return (
		value === "emit-all" ||
		value === "first-win" ||
		value === "longest-win" ||
...
function isTextRulesFeature(value: unknown): value is TextRulesFeature {
	return (
		isRecord(value) &&
...
function isTextRulesTokenPattern(
	value: unknown,
): value is TextRulesTokenPattern {
	return (
		isRecord(value) &&
...
function isTextRulesWhen(value: unknown): value is TextRulesRuleWhenV1 {
	return (
		...
		(isRecord(value) &&
			...
			(value.annotationKind === undefined ||
...
function ruleProvenance(rule: TextRulesRuleDeclarationV1): TextRulesProvenance {
	return {
		references: uniqueReferences([
			...resourceReferences(rule),
			ruleReference(rule),
		]),
	};
}
...
export function compileTextRulesFromTextPackResources(
	...
	for (const resourceId of [...new Set(options.requiredResourceIds ?? [])].sort(
...
interface TextRulesTextPackCandidateMatch
	extends Omit<TextRulesTextPackRuleMatch, "rule"> {}
...
function textPackRuleCandidateText(
	...
	if (document.text !== undefined)
		return document.text.slice(first.startCU, last.endCU);
	return tokens
...
function textPackCandidateMatches(
	...
	for (let startIndex = 0; startIndex < tokens.length; startIndex += 1) {
		for (
			...
			const last = tokens[endIndexExclusive - 1];
			if (first === undefined || last === undefined) continue;
			const candidate = textPackRuleCandidateText(
				document,
				tokens,
				startIndex,
				endIndexExclusive,
...
function textPackRuleReferences(
	rule: TextRulesTextPackCompiledRule,
): readonly TextRulesReferenceRef[] {
	return uniqueReferences([
		...
		{
			kind: "textpack-entry",
			id: `${textPackRuleResourceKey(rule)}:${rule.line}`,
		},
...
export function runTextPackRulesOverTextDoc(
	input: TextRulesTextPackRunInput,
): TextRulesRunResult {
	const workingDocument = textRulesWorkingDocumentFromFinal(input.document);
	...
	const maxRuleValueLength = Math.max(
		0,
		...input.compiled.rules.map((rule) => rule.value.length),
	);
	const matches = textPackCandidateMatches(
		workingDocument,
		tokens,
		maxRuleValueLength,
	)
		.flatMap((candidate) =>
			(rulesByValue.get(candidate.text) ?? []).map((rule) => ({
				...candidate,
				rule,
			})),
		)
		.sort(compareTextPackRuleMatches);
	const annotations = matches.map((match, index) =>
		textPackRuleAnnotationForMatch(match, index),
	);
	const viewId =
		tokens[0]?.viewId ?? workingDocument.views[0]?.id ?? "source-view";
	const extensionLayer: TextRulesLayer<TextRulesExtensionAnnotation> = {
	...
	return {
		document: finalDocumentFromTextRules(
			{
				...workingDocument,
				layers: [
					...workingDocument.layers.filter(
						(layer) => layer.id !== extensionLayer.id,
					),
			...
			{
				base: input.document,
				replaceLayerIds: [extensionLayer.id],
			},
		),
		annotations,
		diagnostics: [],
...
export function createTextPackRulesPipelineProcessor(
	...
	const outputLayerId =
		options.outputLayerId ?? "textrules:textpack-rule-outputs";
	...
	return {
		...
		run(document) {
			const compilation = compileTextRulesFromTextPackResources(
				options.resources,
				options.requiredResourceIds === undefined
					? {}
					: { requiredResourceIds: options.requiredResourceIds },
			);
			const errorMessage = textPackRuleCompilationErrorMessage(
				compilation.diagnostics,
			);
			if (errorMessage.length > 0) {
				throw new TypeError(
					`textrules textpack processor missing required resources: ${errorMessage}`,
				);
			}
			...
			return {
				document: {
					...
					metadata: {
						...
						revision: `${textRulesDocumentRevision(document)}>${id}`,
					},
				},
				diagnostics: [
...
function normalizeRuleDeclaration(
	rule: TextRulesRuleDeclarationV1,
): TextRulesRuleDeclarationV1 {
	return {
		...rule,
		namespace: normalizeSurface(rule.namespace),
		enabled: rule.enabled !== false,
		resources: [...(rule.resources ?? [])].sort((left, right) =>
			left.localeCompare(right),
		),
	};
}
...
export function parseTextRulesRuleBundle(
	value: unknown,
): TextRulesRuleBundleV1 {
...
export function validateTextRulesRuleBundle(
	value: unknown,
): TextRulesRuleValidationResult {
	...
	if (
		...
		!isTextRulesConflictPolicy(value.conflictPolicy)
	) {
		diagnostics.push({
			code: "invalid-conflict-policy",
			severity: "error",
			message:
				"Rule bundle conflictPolicy must be emit-all, first-win, longest-win, or error.",
		});
	}
	...
	const bundleResourceList = isStringArray(value.resources)
		? value.resources
		: [];
	...
	for (const ruleValue of value.rules) {
		...
		if (
			...
			(ruleValue.resources !== undefined &&
				!isStringArray(ruleValue.resources)) ||
			(ruleValue.requires !== undefined &&
				!isStringArray(ruleValue.requires)) ||
			(ruleValue.enabled !== undefined &&
				typeof ruleValue.enabled !== "boolean") ||
			(ruleValue.diagnostic !== undefined &&
				typeof ruleValue.diagnostic !== "boolean")
		) {
			diagnostics.push({
				code: "invalid-rule",
				severity: "error",
		...
		if (
			ruleValue.kind !== "rewrite" &&
			ruleValue.emit === undefined &&
			ruleValue.diagnostic !== true
		) {
			diagnostics.push({
				code: "invalid-rule",
				severity: "error",
...
export function compileTextRulesRuleBundle(
	...
	const resourceIds = [
		...new Set([
			...(parsed.resources ?? []),
			...rules.flatMap((rule) => rule.resources ?? []),
		]),
	].sort((left, right) => left.localeCompare(right));
	const sealed = {
...
function compareRuleMatches(
	...
	const leftLength =
		left.match.endTokenIndexExclusive - left.match.startTokenIndex;
	const rightLength =
		right.match.endTokenIndexExclusive - right.match.startTokenIndex;
	return (
...
function applyConflictPolicy(
	...
	for (const candidate of ordered) {
		const overlaps = selected.some(
			(entry) =>
				candidate.match.startCU < entry.match.endCU &&
				entry.match.startCU < candidate.match.endCU,
		);
		if (!overlaps) {
...
function extensionAnnotationForMatch(
	...
	return {
		...
		lifecycle: { state: "active" },
		targets: [
			{ kind: "span", viewId, startCU: match.startCU, endCU: match.endCU },
		],
...
function annotationMatchesRule(
	annotation: TextRulesAnnotation,
	rule: TextRulesRuleDeclarationV1,
): boolean {
	if (
		rule.when?.annotationKind !== undefined &&
		annotation.kind !== rule.when.annotationKind
	)
		return false;
	if (
		rule.reads?.annotationKind !== undefined &&
		annotation.kind !== rule.reads.annotationKind
	)
		return false;
	const feature = rule.when?.feature;
	if (feature === undefined) return true;
	if (annotation.kind !== "pos" && annotation.kind !== "morphology")
		return false;
	const text = stableJson(annotation);
	return (
		text.includes(`"${feature.name}"`) && text.includes(`"${feature.value}"`)
	);
}

function tokenSurfaceMatches(
	rule: TextRulesRuleDeclarationV1,
	token: TextRulesTokenSpan,
): boolean {
	...
	return surfaces.some(
		(surface) => normalizeSurface(surface) === normalizeSurface(token.text),
	);
}
...
function lexiconRuleCandidates(
	...
	const tokens = textRulesTokenSpansFromWorkingDocument(document, options);
	const surfaces = new Set<string>();
	for (const resource of runtimeResourcesForRule(rule, resources)) {
		for (const entry of resource.entries ?? [])
			surfaces.add(normalizeSurface(entry));
		for (const entry of resource.lexicon?.entries ?? [])
			surfaces.add(normalizeSurface(entry.surface));
	}
	const declared = rule.when?.surfaceIn ?? [];
	for (const surface of declared) surfaces.add(normalizeSurface(surface));
	return tokens.flatMap((token, index) => {
		if (surfaces.size > 0 && !surfaces.has(normalizeSurface(token.text)))
			return [];
...
function ruleCandidates(
	...
	for (const rule of compiled.rules) {
		...
		if (
			rule.kind === "lexicon" ||
			(rule.kind === "transducer" && rulePattern(rule) === undefined)
		) {
			candidates.push(
				...lexiconRuleCandidates(document, rule, resources, options),
			);
			continue;
		}
		const pattern = rulePattern(rule);
		if (pattern === undefined) continue;
		for (const match of matchTextRulesTokenPattern(tokens, {
			...pattern,
			ruleId: rule.id,
		})) {
...
function annotationPatternAnnotations(
	...
	rule: TextRulesRuleDeclarationV1,
	viewId: string,
): readonly TextRulesExtensionAnnotation[] {
	const annotations = document.layers
		.flatMap((layer) => layer.annotations)
		.filter((annotation) => annotationMatchesRule(annotation, rule));
	return annotations.map((annotation, index) => ({
		id: `${rule.namespace}:${rule.id}:annotation-${index + 1}`,
		kind: "extension",
		extensionId:
			rule.emit?.extensionId ?? `${rule.namespace}:annotation-pattern`,
		lifecycle: { state: "active" },
...
function transducerAnnotationForMatch(
	...
	const analyses = [...(rule.emit?.transducerAnalyses ?? [])].sort(
	...
	return {
		...
		kind: "extension",
		extensionId:
			rule.emit?.extensionId ?? `${rule.namespace}:transducer-analysis`,
		lifecycle: { state: "active" },
		targets: [
			{ kind: "span", viewId, startCU: match.startCU, endCU: match.endCU },
		],
...
function validationDiagnosticForMatch(
	...
	return {
		...
		severity:
			rule.emit?.diagnosticSeverity ??
			(rule.diagnostic === true ? "warning" : "info"),
...
function rewriteArtifactsForMatches(
	...
	const rewriteMatches = matches.filter(
		(entry) =>
			entry.rule.kind === "rewrite" && entry.rule.rewrite !== undefined,
	);
	const views: TextRulesView[] = [];
	const spanMaps: TextRulesSpanMap[] = [];
	const rewrites: TextRulesRewriteArtifact[] = [];
	const sourceViewId =
		options.sourceViewId ?? document.views[0]?.id ?? "source-view";
	for (const entry of rewriteMatches) {
		...
		if (
			!document.views.some((view) => view.id === targetViewId) &&
			!views.some((view) => view.id === targetViewId)
		) {
			views.push({
				id: targetViewId,
				kind: "normalized",
		...
		spanMaps.push({
			...
			targetViewId,
			lifecycle: {
				state: rewrite.reversible === false ? "partial" : "active",
				...(rewrite.reversible === false
					? { reason: "rewrite declared non-reversible output" }
					: {}),
			},
			segments: [
				{
...
export function runTextRules(
	...
	const { selected, diagnostics: conflictDiagnostics } = applyConflictPolicy(
		candidates,
		compiled.conflictPolicy,
	);
	const annotations: TextRulesExtensionAnnotation[] = [];
	...
	for (const rule of compiled.rules.filter(
		(entry) => entry.kind === "annotation-pattern",
	)) {
	...
	const layers: TextRulesLayer<TextRulesAnnotation>[] = [
...
function segmentSentencesForRules(
	text: string,
): readonly TextRulesSentenceSpan[] {
	...
	for (let cursor = 0; cursor < text.length; cursor += 1) {
		...
		while (
			...
			isWhitespace(text[sentenceStart] ?? "")
		) {
			sentenceStart += 1;
		}
	}
...
function hasRichMorphologyAnalysis(
	analyses: readonly TextRulesResolvedAnalysis[],
): boolean {
	return analyses.some((analysis) => {
		...
		return (
			morphology.length >= 4 ||
			morphology.some((feature) => feature.name.includes("["))
		);
	});
}
...
function createPhenomenonDiagnostics(
	...
	if (
		phenomena.has("unknown-word") &&
		analyses.some((analysis) => analysis.ruleId.startsWith("fallback:"))
	) {
		diagnostics.push({
			code: "unknown-word",
			severity: "warning",
	...
	if (
		phenomena.has("multiword-token") &&
		analysisNotesInclude(analyses, /contraction/iu)
	) {
		diagnostics.push({
	...
	if (
		phenomena.has("historical-spelling") &&
		analysisNotesInclude(analyses, /historical|modernized/iu)
	) {
		diagnostics.push({
...
function createDocumentViews(): readonly TextRulesView[] {
	return [
		{
			id: "source-view",
			kind: "raw",
			description:
				"Original source text for POS, lemma, and morphology analysis.",
		},
		{
			id: "analysis-view",
			kind: "task",
			description:
				"Deterministic textrules POS, lemma, and morphology annotations.",
...
function createAnalysisSpanMaps(
	textLengthCU: number,
): readonly TextRulesSpanMap[] {
	return [
		{
...
function createDependencyParserViews(): readonly TextRulesView[] {
	return [
		{
			id: "source-view",
			kind: "raw",
...
function createRelationExtractionViews(): readonly TextRulesView[] {
	return [
		{
			id: "source-view",
			kind: "raw",
...
function createCoreferenceViews(): readonly TextRulesView[] {
	return [
		{
			id: "source-view",
			kind: "raw",
...
function entityDocumentViews(
	document: TextRulesWorkingDocument,
): readonly TextRulesView[] {
	if (document.views.some((view) => view.id === "analysis-view"))
	...
	return [
		...document.views,
		{
			...
			kind: "task",
			description:
				"Deterministic textrules rule-backed named entity annotations.",
			parentViewId,
			spanMapIds: ["span-map-source-analysis"],
		},
	];
...
function analysisDocumentSpanMaps(
	document: TextRulesWorkingDocument,
): readonly TextRulesSpanMap[] {
	if (
		document.spanMaps?.some(
			(spanMap) => spanMap.id === "span-map-source-analysis",
		)
	) {
...
function sortDiagnostics(
	diagnostics: readonly TextRulesDiagnostic[],
): readonly TextRulesDiagnostic[] {
...
function documentHasLayerKind(
	document: TextRulesWorkingDocument,
	kind: "token" | "sentence",
): boolean {
	return document.layers.some(
		(layer) => layer.kind === kind && layer.annotations.length >= 1,
	);
}
...
const frozenRelationRuleResources: readonly TextRulesRelationRule[] = [
	...
	},
	{
		...
		evidence: [
			{
				captureNames: ["evidence-2", "evidence-3", "evidence-4", "evidence-5"],
			},
		],
	},
	{
...
const frozenCoreferenceRuleResources: readonly TextRulesCoreferenceRule[] = [
	{
		...
		mentions: [
			{ id: "mention-1", kind: "proper", captureNames: ["person"] },
			{
				id: "mention-2",
				kind: "nominal",
				captureNames: ["object-1", "object-2"],
			},
		...
		chains: [
			{
				id: "chain-1",
				mentionIds: ["mention-1", "mention-3"],
				representativeMentionId: "mention-1",
			},
			{
				id: "chain-2",
				mentionIds: ["mention-2", "mention-4"],
				representativeMentionId: "mention-2",
			},
		],
	},
	{
		...
		mentions: [
			{
				id: "mention-1",
				kind: "proper",
				captureNames: ["company-1", "company-2"],
			},
			{
				id: "mention-2",
				kind: "nominal",
				captureNames: ["nominal-1", "nominal-2"],
			},
		],
		chains: [
			{
		...
		chains: [
			{
				...
				representativeMentionId: "mention-1",
				diagnostics: ["singleton-control"],
			},
			{
				id: "chain-2",
				mentionIds: ["mention-2", "mention-3"],
				representativeMentionId: "mention-2",
			},
		],
	},
	{
		...
		mentions: [
			...
			{ id: "mention-2", kind: "proper", captureNames: ["candidate-2"] },
			{
				id: "mention-3",
				kind: "pronoun",
				captureNames: ["pronoun"],
				notes: ["ambiguous-antecedent"],
			},
			{
				id: "mention-4",
				kind: "singleton",
				captureNames: ["file-1", "file-2"],
			},
		],
		chains: [
			{
				...
				mentionIds: ["mention-1"],
				representativeMentionId: "mention-1",
				diagnostics: ["candidate-antecedent-for:mention-3"],
			},
			{
				id: "chain-2",
			...
			},
			{
				...
				diagnostics: ["ambiguous-antecedent"],
			},
			{
				id: "chain-4",
				mentionIds: ["mention-4"],
				representativeMentionId: "mention-4",
				diagnostics: ["singleton-control"],
			},
		],
		diagnostics: [
			{
				...
				severity: "info",
				message:
					"Ambiguous pronoun antecedents are preserved as singleton candidate chains for this frozen slice.",
			},
		],
	},
];

function ruleLanguageMatches(
	ruleLanguage: string | undefined,
	languageHint: string | undefined,
): boolean {
	if (ruleLanguage === undefined) return true;
	return normalizeLanguageHint(languageHint).includes(
		normalizeSurface(ruleLanguage),
	);
}

function capturesByName(
	match: TextRulesPatternMatch,
): ReadonlyMap<string, TextRulesPatternCapture> {
...
function resolveCaptureSpan(
	...
	const startCapture = captureList.reduce((left, right) =>
		left.startCU <= right.startCU ? left : right,
	);
	const endCapture = captureList.reduce((left, right) =>
		left.endCU >= right.endCU ? left : right,
	);
	const startCU = startCapture.startCU + (template.startOffsetCU ?? 0);
	const endCU =
		template.endOffsetCU === undefined
			? endCapture.endCU
			: endCapture.startCU + template.endOffsetCU;
	return {
		text:
			startCapture.value.slice(0, Math.max(0, endCU - startCU)) === ""
				? captureList.map((capture) => capture.value).join(" ")
				: startCapture.value.slice(
						startCU - startCapture.startCU,
						endCU - startCapture.startCU,
					),
		startCU,
		endCU,
	};
...
function resolveCaptureText(
	inputText: string,
	span: Omit<TextRulesRelationSpanSpec, "role">,
): Omit<TextRulesRelationSpanSpec, "role"> {
	return {
		...span,
	...
	for (const rule of rules) {
		...
		const argumentSpecs = rule.arguments.map((argument) => {
			...
			return span === undefined
				? undefined
				: { role: argument.role, ...resolveCaptureText(inputText, span) };
		});
		...
		if (
			argumentSpecs.some((argument) => argument === undefined) ||
			evidenceSpecs.some((evidence) => evidence === undefined)
		) {
			continue;
		}
		specs.push({
			...
			evidence: evidenceSpecs as readonly Omit<
				TextRulesRelationSpanSpec,
				"role"
			>[],
		});
	}
		...
		diagnostics: [
			{
				...
				severity: "warning",
				message:
					"No coreference rule matched the current frozen fixture scope.",
			},
		],
	};
}
...
function dependencySpecsFromDeclarativeRules(
	...
	return applyTextRulesDependencyRules(
		tokens,
		input.languageHint,
		frozenDependencyRuleResources,
	);
}
...
function relationSpecsFromDeclarativeRules(
	...
	return applyTextRulesRelationRules(
		input.text,
		tokens,
		input.languageHint,
		frozenRelationRuleResources,
	);
}
		...
		return [
			{
				...
				severity: "info",
				message:
					"Negated employment wording is preserved as a no-relation control.",
			},
		];
	}
	return [
		{
			...
			severity: "warning",
			message:
				"No relation-extraction rule matched the current frozen fixture scope.",
		},
	];
}
...
function coreferenceSpecsFromDeclarativeRules(
	...
	return applyTextRulesCoreferenceRules(
		input.text,
		tokens,
		input.languageHint,
		frozenCoreferenceRuleResources,
	);
}
...
function createDependencyNodeAnnotations(
	...
	sentenceId: string,
): readonly TextRulesDependencyNodeAnnotation[] {
...
function createDependencyAnnotations(
	specs: readonly TextRulesDependencyNodeSpec[],
	sentenceId: string,
): readonly TextRulesDependencyAnnotation[] {
	return specs.map((spec) => {
		const dependentNodeId = dependencyNodeId(sentenceId, spec.id);
		const headNodeId =
			spec.head === "0" ? null : dependencyNodeId(sentenceId, spec.head);
		return {
			...
			targets: [
				...
				...(headNodeId
					? [{ kind: "annotation" as const, annotationId: headNodeId }]
					: []),
			],
			dependentNodeId,
			headNodeId,
...
function createRelationArgumentAnnotations(
	...
	input: TextRulesRelationExtractionInput,
): readonly TextRulesEntityAnnotation[] {
	return specs.flatMap((spec) => {
		const argumentsForSpec = spec.arguments.map(
			(argument, index): TextRulesEntityAnnotation => ({
		...
		const evidenceForSpec = spec.evidence.map(
			(evidence, index): TextRulesEntityAnnotation => ({
...
function createRelationAnnotations(
	specs: readonly TextRulesRelationSpec[],
): readonly TextRulesRelationAnnotation[] {
	return specs.map((spec) => ({
		id: spec.id,
		kind: "relation",
		lifecycle: {
...
function createCoreferenceMentionAnnotations(
	...
	input: TextRulesCoreferenceInput,
): readonly TextRulesCoreferenceMentionAnnotation[] {
	return specs.map((spec) => ({
		id: spec.id,
...
): readonly TextRulesCoreferenceChainAnnotation[] {
	return specs.map((spec) => ({
		id: spec.id,
		...
		...(spec.representativeMentionId
			? { representativeMentionId: spec.representativeMentionId }
			: {}),
		...(spec.diagnostics && spec.diagnostics.length > 0
			? { notes: spec.diagnostics }
			: {}),
		provenance: {
			references: [
				{
...
function hasEntityBoundary(
	text: string,
	startCU: number,
	endCU: number,
): boolean {
	return (
		isBoundaryCodeUnit(text[startCU - 1]) && isBoundaryCodeUnit(text[endCU])
	);
}
...
function compareEntityMatches(
	left: TextRulesEntityMatch,
	right: TextRulesEntityMatch,
): number {
	return (
		left.startCU - right.startCU ||
	...
	for (const resource of resources) {
		for (const entry of resource.entries) {
			...
			for (const surface of surfaces) {
				...
				const exactMatches = findSurfaceMatches(
					text,
					surface,
					entry,
					resource,
...
function collectEntityMatchesFromTextDoc(
	...
	if (document.text === undefined) {
		throw new TypeError(
			"entity matching over textdoc tokens requires document.text",
		);
	}
	const tokens = textRulesTokenSpansFromWorkingDocument(document);
	const matchesByKey = new Map<string, TextRulesEntityMatch>();
			...
			for (const surface of [entry.surface, ...(entry.aliases ?? [])]) {
				...
				for (let startIndex = 0; startIndex < tokens.length; startIndex += 1) {
					...
					for (
						...
						const candidateText = document.text.slice(
							startToken.startCU,
							endToken.endCU,
						);
...
function filterEntityOverlaps(
	...
	for (const match of matches) {
		const overlappingMatch = accepted.find((entry) =>
			entityMatchesOverlap(entry, match),
		);
		if (!overlappingMatch) {
			accepted.push(match);
			continue;
	...
	return matches.map((match, index) => ({
		...
		...(match.entry.notes
			? {
					notes: [
						...match.entry.notes,
						`matched-surface:${match.matchedSurface}`,
					],
				}
...
export function isTextRulesLexiconAnalysis(
	value: unknown,
): value is TextRulesLexiconAnalysis {
	return (
		isRecord(value) &&
		isNonEmptyString(value.ruleId) &&
...
export function isTextRulesLexiconEntry(
	value: unknown,
): value is TextRulesLexiconEntry {
	return (
		isRecord(value) &&
...
export function isTextRulesEntityEntry(
	value: unknown,
): value is TextRulesEntityEntry {
	return (
		isRecord(value) &&
		isNonEmptyString(value.id) &&
		isNonEmptyString(value.surface) &&
		...
		(value.caseFoldFallback === undefined ||
			typeof value.caseFoldFallback === "boolean") &&
		(value.notes === undefined || isStringArray(value.notes))
	);
}

export function isTextRulesLexiconResourceData(
	value: unknown,
): value is TextRulesLexiconResourceData {
	return (
		isRecord(value) &&
...
export function isTextRulesEntityResourceData(
	value: unknown,
): value is TextRulesEntityResourceData {
	return (
		isRecord(value) &&
		Array.isArray(value.entries) &&
		value.entries.length >= 1 &&
...
export function createTextRulesEntityResource(
	...
	return {
		...
		entries: data.entries.map((entry) => ({
			...
			...(entry.caseFoldFallback === undefined
				? {}
				: { caseFoldFallback: entry.caseFoldFallback }),
...
function parseTextRulesMorphologyAttributes(
	...
	const features: TextRulesFeature[] = [];
	for (const [name, value] of Object.entries(attributes).sort(
		([left], [right]) => left.localeCompare(right),
	)) {
		if (
			name === "lemma" ||
		...
		resources.push(
			createTextRulesLexiconResource(resource, {
				entries: [...entriesBySurface.entries()]
					.sort(([left], [right]) =>
						normalizeSurface(left).localeCompare(normalizeSurface(right)),
					)
	...
	for (const loadedResource of loadedResources) {
		...
		for (const entry of loadedResource.entries) {
			...
			entries.push({
				...
				surface: entry.value,
				label: entry.label,
				...(entry.attributes.normalized === undefined
					? {}
					: { normalized: entry.attributes.normalized }),
...
export function analyzeRuleBackedNer(
	...
	const document = textRulesWorkingDocumentFromFinal(input.document);
	if (document.text === undefined) {
		throw new TypeError(
			"rule-backed NER requires document.text so offsets can be matched deterministically",
		);
	}
	...
	const selectedResources = selectEntityResources(
		resources,
		input.languageHint,
	);
	const rawMatches = collectEntityMatchesFromTextDoc(
		document,
		selectedResources,
	);
	const { matches, diagnostics } = filterEntityOverlaps(
		rawMatches,
		input.allowSpanOverlap === true,
	);
		...
		document: finalDocumentFromTextRules(
			{
				...
				layers: [
					...document.layers.filter((layer) => layer.id !== "entities"),
					entityLayer,
				],
			},
			{
				base: input.document,
				replaceLayerIds: [entityLayer.id],
			},
		),
		diagnostics: sortDiagnostics(diagnostics),
	};
...
export function analyzePosMorphLemmaDocument(
	...
	const source = workingDocument.source ?? { id: workingDocument.documentId };
	const posAnnotations: TextRulesPosAnnotation[] = [];
	const lemmaAnnotations: TextRulesLemmaAnnotation[] = [];
	const morphologyAnnotations: TextRulesMorphologyAnnotation[] = [];
	const diagnostics: TextRulesDiagnostic[] = [];
	...
	if ((input.phenomena ?? []).includes("code-switching")) {
		diagnostics.push({
			...
			severity: "info",
			message:
				"Mixed-language slice loads lexicon resources for each declared language hint.",
		});
	}

	for (const token of tokens) {
		const { analyses, diagnostics: fallbackDiagnostics } = resolveAnalyses(
			token,
			entriesBySurface,
		);
		diagnostics.push(
		...
		const references = analyses.flatMap((analysis) => [
			...analysis.resourceRefs,
			{
				kind: "textrules-rule",
				id: analysis.ruleId,
			} satisfies TextRulesReferenceRef,
		]);
		...
		document: finalDocumentFromTextRules(
			{
				...
				views: posMorphLemmaViews(workingDocument),
				spanMaps: analysisDocumentSpanMaps(workingDocument),
				layers,
				...(input.phenomena && input.phenomena.length > 0
					? {
							notes: input.phenomena.map(
			...
			{
				base: input.document,
				replaceLayerIds: ["pos", "lemmas", "morphology"],
			},
		),
		diagnostics: sortDiagnostics(diagnostics),
	};
...
export function analyzePosMorphLemma(
	...
	const document: TextRulesWorkingDocument = {
		schemaVersion: documentSchemaVersion,
		...
		layers: [
			{
				...
				kind: "token",
				viewId: "analysis-view",
				annotations: createTokenAnnotations(
					tokens,
					input.sourceId,
					input.sourceSha256,
				),
			},
			{
				...
				kind: "sentence",
				viewId: "analysis-view",
				annotations: createSentenceAnnotations(
					sentences,
					input.sourceId,
					input.sourceSha256,
				),
	...
	return analyzePosMorphLemmaDocument(
		{
			document: finalDocumentFromTextRules(document),
			revision: input.revision ?? posMorphLemmaRevision,
...
export function analyzeRelationExtraction(
	...
	const diagnostics = relationDiagnosticsFromRules(
		tokens,
		relationSpecs.length,
	);

	const tokenLayer: TextRulesLayer<TextRulesTokenAnnotation> = {
		id: "tokens",
		kind: "token",
		viewId: "analysis-view",
		annotations: createTokenAnnotations(
	...
	const sentenceLayer: TextRulesLayer<TextRulesSentenceAnnotation> = {
		id: "sentences",
		kind: "sentence",
		viewId: "analysis-view",
		annotations: createSentenceAnnotations(
	...
	const relationArgumentLayer: TextRulesEntityLayer = {
		...
		notes: [
			"Relation argument spans for frozen relation-extraction fixture scope.",
		],
	};
...
export function analyzeCoreference(
	input: TextRulesCoreferenceInput,
): TextRulesCoreferenceResult {
	const tokens = tokenizeTextForRules(input.text);
	const sentences = segmentSentencesForRules(input.text);
	...
	const mentionLayer: TextRulesCoreferenceMentionLayer = {
		...
		kind: "coreference-mention",
		viewId: "analysis-view",
		annotations: createCoreferenceMentionAnnotations(
			coreferenceSpec.mentions,
			input,
		),
...
export function analyzeDependencyParser(
	...
	const dependencySpecs =
		sentence === undefined
			? []
			: dependencySpecsFromDeclarativeRules(input, tokens);
	const diagnostics: TextRulesDiagnostic[] = [];
	...
	if (dependencySpecs.length === 0) {
		diagnostics.push({
			...
			severity: "warning",
			message:
				"No dependency-parser rule matched the current frozen fixture scope.",
		});
	}
	...
	const dependencyLayer: TextRulesDependencyLayer = {
		...
		annotations:
			sentence === undefined
				? []
				: createDependencyAnnotations(dependencySpecs, sentence.id),
	...
	const layers: TextRulesLayer<TextRulesAnnotation>[] = [
		...
		...(dependencySpecs.length > 0
			? [dependencyNodeLayer, dependencyLayer]
			: []),
	];
