import type { TextPackManifest } from "@ismail-elkorchi/textpack";

export const manifest: TextPackManifest = {
	id: "pack:tr-morphology",
	name: "Turkish Morphology Reference Pack",
	version: "0.1.0",
	packageName: "@ismail-elkorchi/textpack-tr-morphology",
	kind: ["morphology", "fst", "lexicon"],
	targets: { languages: ["tr"], scripts: ["Latn"], modalities: ["typed"] },
	engines: { "@ismail-elkorchi/textpack": "^0.1.0" },
	resources: [
		{
			id: "morph-tr-demo",
			kind: "morphology",
			path: "resources/morph.tr.demo.tsv",
			format: "tsv",
		},
		{
			id: "fst-tr-demo",
			kind: "fst",
			path: "resources/fst.tr.demo.txt",
			format: "lines",
		},
		{
			id: "lexicon-tr-demo",
			kind: "lexicon",
			path: "resources/lexicon.tr.demo.tsv",
			format: "tsv",
		},
	],
	capabilities: { morphology: "fst", normalization: "rules" },
	license: "MIT",
	citations: ["repo:packages/textpacks/textpack-tr-morphology/resources"],
} as const;
