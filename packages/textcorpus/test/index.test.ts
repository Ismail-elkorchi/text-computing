...
import {
	addAnnotation,
	addLayer,
	createDocument as createTextDocument,
	type Annotation,
	type TextDocument,
	type TokenValue,
...
function createDocument(
	...
	let cursor = 0;
	const annotations: Array<Annotation<TokenValue>> = [];
			...
			spans: [
				{
					...
					span: { start: startCU, end: endCU, unit: "utf16-code-unit" },
				},
			],
			value: { index, text: tokenText },
			evidence: {
				mode: "algorithm",
				exactness: "E1",
				...
				inputViewIds: ["analysis-view"],
			},
		});
	}

	let document = addLayer(
		createTextDocument(text, {
			id: documentId,
			rawViewId: "analysis-view",
			metadata: { revision },
		}),
		{
			id: "tokens",
	...
	for (const annotation of annotations) {
		document = addAnnotation(document, annotation);
	}
	return document;
}
...
const alphaEntry: TextCorpusEntry = {
	id: "doc-a",
	document: createDocument("doc:a", "r1", "alpha beta gamma", [
		"alpha",
		"beta",
		"gamma",
	]),
...
const betaEntry: TextCorpusEntry = {
	id: "doc-b",
	document: createDocument("doc:b", "r1", "alpha beta delta", [
		"alpha",
		"beta",
		"delta",
	]),
...
const gammaEntry: TextCorpusEntry = {
	id: "doc-c",
	document: createDocument("doc:c", "r1", "bonjour corpus monde", [
		"bonjour",
		"corpus",
		"monde",
	]),
...
const collection = createTextCorpusCollection(
	[gammaEntry, alphaEntry, betaEntry],
	{
...
if (collection.schemaVersion !== textCorpusCollectionSchemaVersion) {
	throw new Error(
		"textcorpus collection should use the collection schema version",
	);
}
...
if (
	collection.entries.map((entry) => entry.id).join(",") !== "doc-a,doc-b,doc-c"
) {
	throw new Error(
		"textcorpus collection entries should be deterministically sorted by id",
	);
}
...
try {
	...
	duplicateDocumentRejected =
		...
		error.message ===
			`duplicate textcorpus documentId: ${alphaEntry.document.id}`;
}
...
try {
	...
	missingViewRejected =
		error instanceof Error &&
		error.message === "entry doc-a references missing view missing-view";
}

if (!missingViewRejected) {
	throw new Error(
		"textcorpus should reject entries that reference missing views",
	);
}
...
if (!missingLayerRejected) {
	throw new Error(
		"textcorpus should reject entries that reference missing token layers",
	);
}

const nonUtf16TokenAnnotation: Annotation<{ readonly index: number }> = {
	id: "token-emoji",
	layer: "tokens",
	type: "token.word",
	spans: [
...
const nonUtf16Document = addAnnotation(
	addLayer(
		createTextDocument("😀", {
			id: "doc:non-utf16",
			rawViewId: "analysis-view",
	...
	nonUtf16TokenAnnotation,
);
const nonUtf16Collection = createTextCorpusCollection(
	[
		{
			id: "doc-non-utf16",
			document: nonUtf16Document,
			viewId: "analysis-view",
			tokenLayerId: "tokens",
		},
	],
	{ corpusId: "corpus:non-utf16" },
);
let nonUtf16SpanRejected = false;
try {
	computeTextCorpusFrequencies(nonUtf16Collection);
} catch (error) {
	nonUtf16SpanRejected =
		error instanceof Error &&
		error.message ===
			"token annotation token-emoji uses unsupported grapheme span unit for source text slicing";
}
if (!nonUtf16SpanRejected) {
	throw new Error(
		"textcorpus should reject non-UTF-16 token spans before slicing source text",
	);
}
...
if (slicedCollection.entries.map((entry) => entry.id).join(",") !== "doc-a") {
	throw new Error(
		"metadata slicing should preserve deterministic filtered ordering",
	);
}
...
if (
	...
	concordance.rows
		.map(
			(row) =>
				`${row.docId}:${row.tokenIndex}:${row.left.join(" ")}|${row.match}|${row.right.join(" ")}`,
		)
		.join(",") !== "doc-a:1:alpha|beta|gamma,doc-b:1:alpha|beta|delta"
) {
	throw new Error(
		"concordance should expose exact-token KWIC rows with selection provenance",
	);
}

const frequency = computeTextCorpusFrequencies(collection, {
	metadataFilters: { language: "en" },
});
...
if (
	frequency.rows
		.map((row) => `${row.term}:${row.count}:${row.documentFrequency}`)
		.join(",") !== "alpha:2:2,beta:2:2,delta:1:1,gamma:1:1"
) {
	throw new Error(
		"frequency output should expose deterministic raw and document-frequency counts",
	);
}

const frequencyMetricPayload = exportTextCorpusMetricEnvelopePayloadV1(
	frequency,
	{
...
if (!isTextCorpusMetricEnvelopePayloadV1(frequencyMetricPayload)) {
	throw new Error(
		"frequency metric payload should satisfy the corpus metric payload contract",
	);
}
if (
	frequencyMetricPayload.metrics
		.map((entry) => `${entry.metricId}:${entry.kind}:${entry.value}`)
		.join(",") !==
	...
	throw new Error(
		"frequency metric payload should expose selection and frequency metrics deterministically",
	);
}

const ngrams = computeTextCorpusNgrams(collection, {
	n: 2,
	metadataFilters: { language: "en" },
});
...
if (
	ngrams.rows
		.map(
			(row) => `${row.ngram.join(" ")}:${row.count}:${row.documentFrequency}`,
		)
		.join(",") !== "alpha beta:2:2,beta delta:1:1,beta gamma:1:1"
) {
	throw new Error(
		"n-gram output should expose deterministic exact-token n-grams",
	);
}
...
if (
	cooccurrences.rows
		.map((row) => `${row.term}:${row.coTerm}:${row.count}`)
		.join(",") !== "alpha:beta:2,beta:delta:1,beta:gamma:1"
) {
	throw new Error(
		"co-occurrence output should count deterministic unordered token windows",
	);
}
...
if (
	collocates.rows
		.map((row) => `${row.term}:${row.coTerm}:${row.count}`)
		.join(",") !== "beta:alpha:2,beta:delta:1,beta:gamma:1"
) {
...
const relationCorpus = createTextCorpusCollection(
	[
		...
		betaEntry,
		{
			...alphaEntry,
			id: "doc-a-copy",
			document: createDocument("doc:a-copy", "r1", "alpha beta gamma", [
				"alpha",
				"beta",
				"gamma",
			]),
...
if (!isTextCorpusPairwiseRelationResultV1(pairwiseRelations)) {
	throw new Error(
		"pairwise relation output should satisfy the runtime contract",
	);
}
...
if (
	pairwiseRelations.rows
		.map(
			(row) =>
				`${row.leftDocId}/${row.rightDocId}:${row.relation}:${row.jaccard}`,
		)
		.join(",") !==
	"doc-a/doc-a-copy:exact-duplicate:1,doc-a/doc-b:near-duplicate:0.3333333333333333,doc-a-copy/doc-b:near-duplicate:0.3333333333333333"
) {
	throw new Error(
		"pairwise relation output should expose deterministic reuse relations",
	);
}

const serializedFrequency = stringifyTextCorpusArtifact(frequency);
if (
	JSON.stringify(parseTextCorpusArtifact(serializedFrequency)) !==
	JSON.stringify(frequency)
) {
	throw new Error(
		"generic corpus artifact persistence should round-trip frequency output deterministically",
	);
}
...
if (!invalidArtifactRejected) {
	throw new Error(
		"generic corpus artifact parser should reject unknown persisted artifacts",
	);
}
...
if (!mismatchedArtifactRejected) {
	throw new Error(
		"generic corpus artifact persistence should reject mismatched selection provenance",
	);
}
...
try {
	...
	emptyConcordanceQueryRejected =
		error instanceof TypeError &&
		error.message === "textcorpus concordance query must be a non-empty string";
}
...
try {
	...
	invalidNgramRejected =
		error instanceof TypeError &&
		error.message === "textcorpus n-gram size must be a positive integer";
}
...
if (!isTextCorpusFingerprintIndex(fingerprintIndex)) {
	throw new Error(
		"textcorpus fingerprint index should satisfy the runtime contract",
	);
}
...
if (
	JSON.stringify(fingerprintIndex) !== JSON.stringify(repeatedFingerprintIndex)
) {
	throw new Error(
		"fingerprint index should be deterministic for identical inputs",
	);
}
...
if (!sharedDocIds) {
	throw new Error(
		"fingerprint index should record shared shingles across documents",
	);
}
...
const scoringAlphaEntry: TextCorpusEntry = {
	...alphaEntry,
	document: createDocument("doc:score-a", "r1", "alpha beta beta", [
		"alpha",
		"beta",
		"beta",
	]),
...
const scoringBetaEntry: TextCorpusEntry = {
	...betaEntry,
	document: createDocument("doc:score-b", "r1", "alpha gamma", [
		"alpha",
		"gamma",
	]),
};
...
const scoringCollection = createTextCorpusCollection(
	[scoringAlphaEntry, scoringBetaEntry, scoringDeltaEntry, emptyEntry],
	{
...
if (scoringResult.schemaVersion !== textCorpusScoringSchemaVersion) {
	throw new Error(
		"textcorpus scoring result should use the scoring schema version",
	);
}

if (!isTextCorpusScoringResultV1(scoringResult)) {
	throw new Error(
		"textcorpus scoring result should satisfy the runtime contract",
	);
}

if (
	scoringResult.evidenceClass !== "E2" ||
	scoringResult.selection.documentOrder.join(",") !==
		"doc-a,doc-b,doc-c,doc-empty" ||
	scoringResult.selection.tokenCount !== 6
) {
	throw new Error(
		"textcorpus scoring result should disclose E2 selection provenance",
	);
}

if (scoringResult.documentOrder.join(",") !== "doc-a,doc-b,doc-c,doc-empty") {
	throw new Error(
		"scoring output should preserve deterministic collection document order",
	);
}

if (scoringResult.termOrder.join(",") !== "alpha,beta,delta,gamma") {
	throw new Error(
		"scoring output should expose deterministic lexical term order",
	);
}

if (!scoringResult.formulaSet.includes("tfidf.sklearn-smooth-l2")) {
	throw new Error(
		"scoring output should disclose requested TF-IDF formula variants",
	);
}

if (!scoringResult.formulaSet.includes("bm25.okapi.k1-1.2.b-0.75")) {
	throw new Error(
		"scoring output should disclose requested BM25 formula variants",
	);
}
...
function expectNear(
	...
	if (
		actual === undefined ||
		Math.abs(actual - expected) > scoringResult.tolerance
	) {
...
function documentScores(documentId: string) {
	const document = scoringResult.documents.find(
...
function termValue(
	values: readonly { readonly term: string; readonly value: number }[],
	term: string,
): number | undefined {
...
expectNear(
	termValue(documentScores("doc-a").tf, "beta"),
	2,
	"doc-a beta raw tf",
);
expectNear(
...
expectNear(
	termValue(docATfidfL2 ?? [], "beta"),
	0.9303238670444788,
	"doc-a beta l2-normalized smooth tf-idf",
);

if (
	documentScores("doc-empty").length !== 0 ||
	documentScores("doc-empty").tf.length !== 0
) {
...
expectNear(
	queryScore("alpha-beta", "doc-a"),
	0.9159976869050851,
	"alpha-beta BM25 doc-a",
);
expectNear(queryScore("alpha-beta", "doc-b"), 0, "alpha-beta BM25 doc-b");
expectNear(
	queryScore("delta", "doc-c"),
	0.9968210122202397,
	"delta BM25 doc-c",
);
const alphaBetaBm25K1_1_2 = queryScores("alpha-beta").at(0);
...
if (alphaBetaBm25K1_1_2?.docId !== "doc-a") {
	throw new Error(
		"canonical BM25 ordering should remain document-order stable",
	);
}
expectNear(
	alphaBetaVariantScore,
	0.9092952648057798,
	"alpha-beta BM25 k1=1.2 doc-a",
);
...
if (
	JSON.stringify(repeatedScoringResult) !==
	JSON.stringify(repeatedScoringResultAgain)
) {
	throw new Error(
		"corpus scoring should be deterministic for identical inputs",
	);
}
...
try {
	...
	duplicateQueryRejected =
		error instanceof Error &&
		error.message === "duplicate textcorpus query id: alpha-beta";
}
...
if (
	...
	retrievalIndex.selection.documentOrder.join(",") !==
		"doc-a,doc-b,doc-c,doc-empty" ||
...
if (retrievalIndex.termOrder.join(",") !== "alpha,beta,delta,gamma") {
	throw new Error(
		"retrieval index should preserve deterministic term ordering",
	);
}

const parsedQuery = parseTextCorpusQuery("Alpha beta", { id: "alpha-beta" });
if (
	!isTextCorpusParsedQuery(parsedQuery) ||
	parsedQuery.tokens.join(",") !== "alpha,beta"
) {
	throw new Error(
		"query parser should normalize query text to lower-case lexical tokens",
	);
}
...
if (
	...
	retrievalResult.selection.documentOrder.join(",") !==
		retrievalIndex.selection.documentOrder.join(",")
) {
	throw new Error(
		"retrieval result should preserve index selection provenance",
	);
}

function retrievalHits(queryId: string) {
	const queryResult = retrievalResult.results.find(
		(entry) => entry.query.id === queryId,
	);
...
if (alphaBetaHits.map((hit) => hit.docId).join(",") !== "doc-a") {
	throw new Error(
		"alpha-beta retrieval should return only positive-score hits",
	);
}
expectNear(
	alphaBetaHits[0]?.score,
	0.9159976869050851,
	"alpha-beta retrieval doc-a",
);
if (alphaBetaHits[0]?.snippet?.text !== "alpha beta beta") {
	throw new Error("retrieval snippet should expose the local token window");
}
const betaExplain = alphaBetaHits[0]?.explain.find(
	(entry) => entry.term === "beta",
);
expectNear(
	betaExplain?.contribution,
	0.9159976869050851,
...
const repeatedRetrieval = searchTextCorpusRetrievalIndex(
	retrievalIndex,
	[parsedQuery],
	{ topK: 3 },
);
const repeatedRetrievalAgain = searchTextCorpusRetrievalIndex(
	retrievalIndex,
	[parsedQuery],
	{ topK: 3 },
);
if (
	JSON.stringify(repeatedRetrieval) !== JSON.stringify(repeatedRetrievalAgain)
) {
	throw new Error(
		"retrieval output should be deterministic for identical inputs",
	);
}

const serializedRetrievalIndex =
	stringifyTextCorpusRetrievalIndex(retrievalIndex);
const parsedRetrievalIndex = parseTextCorpusRetrievalIndex(
	serializedRetrievalIndex,
);
if (JSON.stringify(parsedRetrievalIndex) !== JSON.stringify(retrievalIndex)) {
	throw new Error(
		"retrieval index persistence should round-trip deterministically",
	);
}
...
if (!invalidRetrievalIndexRejected) {
	throw new Error(
		"retrieval index parser should reject invalid persisted JSON",
	);
}
...
if (
	...
	throw new Error(
		"query parser should expose deterministic fielded/operator clauses",
	);
}

const fieldedRetrieval = searchTextCorpusRetrievalIndex(
	retrievalIndex,
	[fieldedQuery],
	{
		topK: 3,
...
if (fieldedHits.map((hit) => hit.docId).join(",") !== "doc-a") {
	throw new Error(
		"fielded retrieval should combine required, field, and prohibited clauses",
	);
}
...
if (
	fieldOnlyRetrieval.results[0]?.hits.map((hit) => hit.docId).join(",") !==
	"doc-c"
) {
	throw new Error(
		"field-only retrieval should support metadata filters with explicit zero-score inclusion",
	);
}
...
if (!isTextCorpusRetrievalIndexV1(fieldedIndex)) {
	throw new Error(
		"fielded retrieval index should satisfy the runtime contract",
	);
}
...
if (
	...
	fieldedIndex.termOrder.join(",") !==
		"alpha,beta,bulletin,delta,gamma,note,report"
) {
	throw new Error(
		"fielded retrieval index should expose deterministic field and term ordering",
	);
}
...
if (JSON.stringify(fieldedParsed) !== JSON.stringify(fieldedIndex)) {
	throw new Error(
		"fielded retrieval index persistence should round-trip deterministically",
	);
}
...
const bm25fResult = searchTextCorpusRetrievalIndex(
	fieldedIndex,
	[
		parseTextCorpusQuery("title:alpha +beta genre:news", {
			id: "fielded-title-alpha-beta",
		}),
		parseTextCorpusQuery("title:delta genre:note", {
			id: "fielded-delta-note",
		}),
...
if (!isTextCorpusRetrievalResultV1(bm25fResult)) {
	throw new Error(
		"fielded retrieval result should satisfy the runtime contract",
	);
}
...
const fieldedEvaluation = evaluateTextCorpusRetrieval(
	bm25fResult,
	fieldedQrels,
	{
		k: 3,
...
if (
	fieldedEvaluation.schemaVersion !== textCorpusRetrievalEvaluationSchemaVersion
) {
	throw new Error(
		"retrieval evaluation should use the evaluation schema version",
	);
}
...
if (
	...
	fieldedEvaluation.selection.documentOrder.join(",") !==
		bm25fResult.selection.documentOrder.join(",")
) {
	throw new Error(
		"retrieval evaluation should preserve retrieval selection provenance",
	);
}

expectNear(
	fieldedEvaluation.summary.precisionAtK,
	2 / 9,
	"fielded qrels macro precision@3",
);
expectNear(
	fieldedEvaluation.summary.recallAtK,
	2 / 3,
	"fielded qrels macro recall@3",
);
expectNear(fieldedEvaluation.summary.mrr, 2 / 3, "fielded qrels macro MRR");
expectNear(
	fieldedEvaluation.summary.ndcgAtK,
	2 / 3,
	"fielded qrels macro nDCG@3",
);

const negativeEvaluation = fieldedEvaluation.queries.find(
	(entry) => entry.queryId === "negative-field-control",
);
if (
	!negativeEvaluation ||
	...
	throw new Error(
		"retrieval qrels evaluation should preserve zero-relevance negative controls",
	);
}

const evaluationMetricPayload = exportTextCorpusMetricEnvelopePayloadV1(
	fieldedEvaluation,
	{
...
if (
	...
	evaluationMetricPayload.metrics.find(
		(entry) => entry.metricId === "retrieval-evaluation.ndcg-at-k",
	)?.value !==
		2 / 3
) {
	throw new Error(
		"retrieval evaluation metric payload should expose summary metrics",
	);
}
...
try {
	...
	duplicateQrelsDocRejected =
		error instanceof Error &&
		error.message === "duplicate textcorpus qrels doc id: duplicate-doc/doc-a";
}

if (!duplicateQrelsDocRejected) {
	throw new Error(
		"retrieval qrels evaluation should reject duplicate doc ids per query",
	);
}
...
const fieldedTitleAlphaHits =
	bm25fResult.results.find(
		(entry) => entry.query.id === "fielded-title-alpha-beta",
	)?.hits ?? [];
...
if (
	...
	throw new Error(
		"fielded BM25F explain output should record title field contribution",
	);
}
expectNear(
	titleAlphaExplain?.contribution,
	1.1297304805162718,
	"fielded title alpha contribution",
);
const fieldedBetaExplain = fieldedTitleAlphaHits[0]?.explain.find(
	(entry) => entry.term === "beta" && entry.field === undefined,
);
expectNear(
	fieldedBetaExplain?.contribution,
	0.9092952648057796,
	"fielded beta body contribution",
);
expectNear(
	fieldedTitleAlphaHits[0]?.score,
	2.0390257453220513,
	"fielded title/body score",
);
...
if (
	...
	titleBoostProfile.fields
		.map((field) => `${field.field}:${field.weight}`)
		.join(",") !== "body:0.5,title:3"
) {
	throw new Error(
		"retrieval field weight profile should normalize and sort field weights",
	);
}
const boostedFieldedResult = searchTextCorpusRetrievalIndex(
	...
	[
		parseTextCorpusQuery("title:alpha +beta genre:news", {
			id: "fielded-title-alpha-beta-boosted",
		}),
	],
	{
		topK: 3,
...
if (
	boostedFieldedResult.fieldWeightProfile?.profileId !==
		"profile:title-boost" ||
	...
	throw new Error(
		"retrieval field weight profiles should alter BM25F scores with disclosed weights",
	);
}
...
const bodyOnlyFieldedResult = searchTextCorpusRetrievalIndex(
	...
	[
		parseTextCorpusQuery("title:alpha +beta genre:news", {
			id: "fielded-title-alpha-beta-body-only",
		}),
	],
	{
		topK: 3,
...
const bodyOnlyTitleExplain =
	bodyOnlyFieldedResult.results[0]?.hits[0]?.explain.find(
		(entry) => entry.term === "alpha" && entry.field === "title",
	);
if (
	...
	throw new Error(
		"inline retrieval field weights should support zero-weight field controls",
	);
}
...
try {
	searchTextCorpusRetrievalIndex(
		fieldedIndex,
		[parseTextCorpusQuery("alpha", { id: "unknown-field-weight" })],
		{
			fieldWeights: {
				unknown: 1,
			},
	...
	unknownFieldWeightRejected =
		error instanceof Error &&
		error.message ===
			"textcorpus retrieval field weight references unknown field: unknown";
}
...
try {
	searchTextCorpusRetrievalIndex(
		retrievalIndex,
		[parseTextCorpusQuery("alpha", { id: "non-bm25f-weight" })],
		{
			fieldWeights: {
				title: 1,
			},
...
if (!nonBm25fFieldWeightRejected) {
	throw new Error(
		"retrieval field weights should require BM25F retrieval indexes",
	);
}
...
if (
	calibrationReport.schemaVersion !==
	textCorpusRetrievalCalibrationSchemaVersion
) {
	throw new Error(
		"retrieval calibration should use the calibration schema version",
	);
}
if (
	!isTextCorpusRetrievalCalibrationReportV1(calibrationReport) ||
	calibrationReport.selectedCandidateId !== "baseline" ||
	calibrationReport.candidateOrder.join(",") !==
		"baseline,profile:title-boost,profile:zero" ||
	calibrationReport.candidates.length !== 3
) {
	throw new Error(
		"retrieval calibration should rank deterministic profile candidates",
	);
}
const zeroCalibrationCandidate = calibrationReport.candidates.find(
	(entry) => entry.candidateId === "profile:zero",
);
if (
	...
	throw new Error(
		"retrieval calibration should expose degraded zero-weight profile metrics",
	);
}
const calibrationMetricPayload = exportTextCorpusMetricEnvelopePayloadV1(
	calibrationReport,
	{
...
if (
	...
	calibrationMetricPayload.metrics.find(
		(entry) => entry.metricId === "retrieval-calibration.candidate-count",
	)?.value !== 3
) {
	throw new Error(
		"retrieval calibration metric payload should expose profile comparison metrics",
	);
}
const parsedCalibrationReport = parseTextCorpusArtifact(
	stringifyTextCorpusArtifact(calibrationReport),
);
if (!isTextCorpusRetrievalCalibrationReportV1(parsedCalibrationReport)) {
	throw new Error(
		"retrieval calibration should round-trip through generic artifact JSON",
	);
}
const singleCandidateCalibrationReport =
	createTextCorpusRetrievalCalibrationReport(
...
if (
	...
	throw new Error(
		"retrieval calibration constructor should accept caller-provided evaluation candidates",
	);
}
...
try {
	...
	duplicateCalibrationCandidateRejected =
		error instanceof Error &&
		error.message === "duplicate textcorpus retrieval calibration candidate id";
}
if (!duplicateCalibrationCandidateRejected) {
	throw new Error(
		"retrieval calibration should reject duplicate candidate ids",
	);
}
...
if (!nonBm25fCalibrationRejected) {
	throw new Error(
		"retrieval calibration should require BM25F indexes for field-weight profiles",
	);
}
...
const selectedLearningCandidate = fieldWeightLearningReport.candidates.find(
...
if (
	...
	throw new Error(
		"retrieval field-weight learning should generate and rank deterministic BM25F profiles",
	);
}
...
try {
	...
	unknownLearningFieldRejected =
		error instanceof Error &&
		error.message ===
			"textcorpus retrieval field-weight learning references unknown field: unknown";
}
if (!unknownLearningFieldRejected) {
	throw new Error(
		"retrieval field-weight learning should reject unknown fields",
	);
}
...
if (!learningCandidateCapRejected) {
	throw new Error(
		"retrieval field-weight learning should enforce candidate-count caps",
	);
}

const fieldedDeltaHits =
	bm25fResult.results.find((entry) => entry.query.id === "fielded-delta-note")
		?.hits ?? [];
if (fieldedDeltaHits.map((hit) => hit.docId).join(",") !== "doc-c") {
	throw new Error(
		"fielded retrieval should combine indexed title fields and metadata filters",
	);
}

const negativeFieldHits =
	bm25fResult.results.find(
		(entry) => entry.query.id === "negative-field-control",
	)?.hits ?? [];
if (negativeFieldHits.length !== 0) {
	throw new Error(
		"fielded retrieval should reject incompatible required body/title clauses",
	);
}

const booleanPhraseQuery = parseTextCorpusQuery(
	'("alpha beta" OR title:"gamma bulletin") AND -delta',
	{
		id: "boolean-phrase",
	},
);
if (
	...
	booleanPhraseQuery.clauses.filter((clause) => clause.kind === "phrase")
		.length !== 2
) {
	throw new Error(
		"retrieval parser should expose boolean phrase clauses deterministically",
	);
}
const booleanPhraseResult = searchTextCorpusRetrievalIndex(
	fieldedIndex,
	[booleanPhraseQuery],
	{
		topK: 3,
...
if (
	booleanPhraseResult.results[0]?.hits.map((hit) => hit.docId).join(",") !==
	"doc-b,doc-a"
) {
	throw new Error(
		"boolean phrase retrieval should evaluate phrase, field, OR, AND, and NOT semantics",
	);
}

const proximityQuery = parseTextCorpusQuery('"alpha beta"~1', {
	id: "proximity-alpha-beta",
});
if (
	proximityQuery.clauses[0]?.kind !== "proximity" ||
	proximityQuery.clauses[0].proximity !== 1
) {
	throw new Error("retrieval parser should preserve proximity phrase metadata");
}
const proximityResult = searchTextCorpusRetrievalIndex(
	fieldedIndex,
	[proximityQuery],
	{
		topK: 3,
...
if (
	proximityResult.results[0]?.hits.map((hit) => hit.docId).join(",") !== "doc-a"
) {
	throw new Error(
		"proximity retrieval should match ordered terms within the declared distance",
	);
}
...
if (JSON.stringify(streamedFielded) !== JSON.stringify(bm25fResult.results)) {
	throw new Error(
		"streaming retrieval should produce the same query results as batch retrieval",
	);
}

const stoppedStream = iterateTextCorpusRetrievalResults(
	fieldedIndex,
	bm25fResult.results.map((entry) => entry.query),
	{
		topK: 3,
...
if (
	firstStreamed.done ||
	firstStreamed.value.query.id !== "fielded-title-alpha-beta"
) {
	throw new Error(
		"streaming retrieval should allow deterministic early consumption",
	);
}
...
if (!isTextCorpusRetrievalIndexArtifactV1(artifact)) {
	throw new Error(
		"retrieval index artifact should satisfy checksum and shape contracts",
	);
}
const artifactSerialized = stringifyTextCorpusRetrievalIndexArtifact(artifact);
const artifactParsed =
	parseTextCorpusRetrievalIndexArtifact(artifactSerialized);
if (JSON.stringify(artifactParsed) !== JSON.stringify(artifact)) {
	throw new Error(
		"retrieval index artifact should round-trip deterministically",
	);
}
const tamperedArtifact = JSON.parse(artifactSerialized);
tamperedArtifact.index.documentOrder = [
	...tamperedArtifact.index.documentOrder,
].reverse();
...
if (
	...
	storageRef.byteLength !==
		new TextEncoder().encode(artifactSerialized).length ||
	...
	throw new Error(
		"retrieval index storage ref should summarize persisted artifact identity",
	);
}
const storageRefParsed = parseTextCorpusArtifact(
	stringifyTextCorpusArtifact(storageRef),
);
if (JSON.stringify(storageRefParsed) !== JSON.stringify(storageRef)) {
	throw new Error(
		"retrieval index storage ref should round-trip through generic artifact JSON helpers",
	);
}
const storageMetricPayload = exportTextCorpusMetricEnvelopePayloadV1(
	storageRef,
	{
...
if (
	...
	!storageMetricPayload.metrics.some(
		(metric) => metric.metricId === "retrieval-index-storage-ref.byte-length",
	)
) {
	throw new Error(
		"retrieval index storage ref should export metric-envelope payloads",
	);
}
const loadedArtifact = await loadTextCorpusRetrievalIndexArtifactFromStore(
	storageRef,
	{
		readText(key) {
...
if (JSON.stringify(loadedArtifact) !== JSON.stringify(artifact)) {
	throw new Error(
		"retrieval index storage adapter should load the stored artifact",
	);
}
...
if (
	...
	throw new Error(
		"retrieval index filesystem key should be deterministic and corpus/formula scoped",
	);
}
const resolvedFileSystemPath = resolveTextCorpusRetrievalIndexFileSystemPath(
	"/tmp/textcorpus",
	fileSystemKey,
);
if (resolvedFileSystemPath !== `/tmp/textcorpus/${fileSystemKey}`) {
	throw new Error(
		"retrieval index filesystem path resolver should combine root and safe relative key",
	);
}
const fileSystemWrites = new Map<string, string>();
const fileSystemStorageRef =
...
if (
	...
	throw new Error(
		"retrieval index filesystem adapter should persist using package-owned keys",
	);
}
const loadedFileSystemArtifact =
	await loadTextCorpusRetrievalIndexArtifactFromFileSystem(
		fileSystemStorageRef,
		{
			root: "/tmp/textcorpus",
			readText(path) {
...
if (JSON.stringify(loadedFileSystemArtifact) !== JSON.stringify(artifact)) {
	throw new Error(
		"retrieval index filesystem adapter should load package-owned persisted artifacts",
	);
}
const customFileSystemRef =
	await saveTextCorpusRetrievalIndexArtifactToFileSystem(artifact, {
		root: "memory-root",
...
if (
	...
	fileSystemWrites.get("memory-root::indexes/custom-fielded.json") !==
		artifactSerialized
) {
	throw new Error(
		"retrieval index filesystem adapter should allow safe caller-selected keys",
	);
}
...
if (!unsafeFileSystemKeyRejected) {
	throw new Error(
		"retrieval index filesystem adapter should reject path traversal keys",
	);
}
...
if (!byteLengthMismatchRejected) {
	throw new Error(
		"retrieval index storage loader should reject byteLength drift",
	);
}
...
try {
	await loadTextCorpusRetrievalIndexArtifactFromStore(
		{
			...storageRef,
			checksum: { algorithm: "fnv1a64-utf8", value: "0000000000000000" },
		},
		{
			readText(key) {
				return storedIndexArtifacts.get(key) ?? "";
...
if (!checksumMismatchRejected) {
	throw new Error(
		"retrieval index storage loader should reject checksum drift",
	);
}

const citationWindows = createTextCorpusCitationWindows(
	fieldedScoringCollection,
	bm25fResult,
	{
		tokenWindow: 0,
	},
...
const docACitation = citationWindows.windows.find(
	(entry) =>
		entry.queryId === "fielded-title-alpha-beta" && entry.docId === "doc-a",
);
if (
	...
	throw new Error(
		"citation windows should ground retrieval hits to textdoc token spans",
	);
}
...
if (
	...
	throw new Error(
		"quote grounding should preserve ambiguity for repeated token sequences",
	);
}
...
const metricPayloads = [
	...
	exportTextCorpusMetricEnvelopePayloadV1(corpusArtifact),
);

if (
	!metricPayloads.every((payload) =>
		isTextCorpusMetricEnvelopePayloadV1(payload),
	)
) {
	throw new Error(
		"metric payload export should cover all declared textcorpus artifact families",
	);
}

const metricSetIds = metricPayloads
	.map((payload) => payload.metricSetId)
	.join(",");
if (
	metricSetIds !==
	...
	throw new Error(
		`metric payload export should use deterministic default metric set ids: ${metricSetIds}`,
	);
}
...
try {
	...
	emptyMetricSetRejected =
		error instanceof TypeError &&
		error.message === "textcorpus metricSetId must be a non-empty string";
}
...
try {
	...
	duplicateFieldRejected =
		error instanceof Error &&
		error.message === "duplicate textcorpus retrieval field id: title";
}
...
const largeEntries = Array.from(
	{ length: 128 },
	(_, index): TextCorpusEntry => {
		const id = `large-${String(index).padStart(3, "0")}`;
		const hasTarget = index % 17 === 0;
		const tokens = hasTarget
			? ["target", "common", `term-${index}`]
			: ["common", `term-${index}`];
		return {
			id,
...
if (
	largeIndex.documentOrder.length !== 128 ||
	largeIndex.termOrder.length !== 130
) {
	throw new Error(
		"large retrieval fixture should preserve all documents and deterministic terms",
	);
}

if (
	largeResult.results[0]?.hits.map((hit) => hit.docId).join(",") !==
	"large-000,large-034,large-068,large-102"
) {
	throw new Error(
		"large retrieval fixture should preserve deterministic topK ordering under filters",
	);
}
...
if (
	...
	throw new Error(
		"large fielded retrieval fixture should preserve deterministic topK ordering",
	);
}
