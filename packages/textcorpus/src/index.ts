import {
	stableHash64,
	type StableHash64Algorithm,
} from "@ismail-elkorchi/textfacts/hash";
import {
	isTextDocument,
	type Annotation,
...
export const textCorpusTfidfSklearnSmoothRawFormula =
	"tfidf.sklearn-smooth-raw" as const;
export const textCorpusTfidfSklearnSmoothL2Formula =
	"tfidf.sklearn-smooth-l2" as const;
export const textCorpusBm25OkapiFormula = "bm25.okapi.k1-1.5.b-0.75" as const;
export const textCorpusBm25OkapiK1_1_2Formula =
	"bm25.okapi.k1-1.2.b-0.75" as const;
export const textCorpusBm25fFormula = "bm25f.k1-1.2.b-0.75.fielded" as const;

export type PackageName = typeof packageName;
export type TextCorpusCollectionSchemaVersion =
	typeof textCorpusCollectionSchemaVersion;
export type TextCorpusScoringSchemaVersion =
	typeof textCorpusScoringSchemaVersion;
export type TextCorpusRetrievalSchemaVersion =
	typeof textCorpusRetrievalSchemaVersion;
export type TextCorpusRetrievalQrelsSchemaVersion =
	typeof textCorpusRetrievalQrelsSchemaVersion;
...
export type TextCorpusAnalysisSchemaVersion =
	typeof textCorpusAnalysisSchemaVersion;
...
type TextCorpusTokenAnnotation = Annotation<TokenValue>;
type TextCorpusTokenLayer = AnnotationLayer<TokenValue>;
...
export type TextCorpusNormalizedMetadataFilters = Readonly<
...
export interface TextCorpusRetrievalIndexV1 {
	...
	readonly invertedIndex: Readonly<
		Record<string, readonly TextCorpusPosting[]>
	>;
...
export interface TextCorpusRetrievalStreamOptions
	extends TextCorpusRetrievalSearchOptions {
...
export interface TextCorpusConcordanceOptions
	extends TextCorpusAnalysisSelectionOptions {
...
export interface TextCorpusFrequencyOptions
	extends TextCorpusAnalysisSelectionOptions {}
...
export interface TextCorpusNgramOptions
	extends TextCorpusAnalysisSelectionOptions {
	readonly n: number;
}
...
export interface TextCorpusCooccurrenceOptions
	extends TextCorpusAnalysisSelectionOptions {
	readonly window?: number;
}
...
export interface TextCorpusCollocateOptions
	extends TextCorpusCooccurrenceOptions {
	readonly term: string;
}
...
export interface TextCorpusPairwiseRelationOptions
	extends TextCorpusAnalysisSelectionOptions {
	readonly shingleSize: number;
	readonly windowSize: number;
	readonly hashAlgorithm?: StableHash64Algorithm;
...
export type TextCorpusMetricEnvelopeMetricParameters = Readonly<
...
function isTokenLayer(
	...
	tokenLayerId: string,
	viewId: string,
): layer is TextCorpusTokenLayer {
	return (
		layer.id === tokenLayerId &&
		layer.viewId === viewId &&
...
function findTokenLayer(
	...
	tokenLayerId: string,
	viewId: string,
): TextCorpusTokenLayer | undefined {
	const layer = document.layers[tokenLayerId];
	return layer !== undefined && isTokenLayer(layer, tokenLayerId, viewId)
		? layer
		: undefined;
}

function compareEntriesById(
	left: TextCorpusEntry,
	right: TextCorpusEntry,
): number {
	return left.id.localeCompare(right.id);
}

function rightmostMinimumIndex(
...
function selectFingerprintIndexes(
	...
	if (hashes.length <= effectiveWindow) {
		for (
			let index = 0;
			index < hashes.length && selected.size < maxFingerprintsPerDoc;
			index += 1
		) {
			selected.add(index);
		}
		...
		for (
			let startIndex = 0;
			startIndex <= hashes.length - effectiveWindow;
			startIndex += 1
		) {
			selected.add(
				rightmostMinimumIndex(hashes, startIndex, startIndex + effectiveWindow),
			);
	...
	return Array.from(selected)
		...
		.slice(0, maxFingerprintsPerDoc);
}

function resolveTokenText(
	document: TextDocument,
	annotation: TextCorpusTokenAnnotation,
): string {
	if (annotation.value?.text !== undefined) return annotation.value.text;
	const spanTarget = annotation.spans[0];
	if (spanTarget === undefined) {
		throw new Error(`token annotation ${annotation.id} must target a span`);
	}
	const view = document.views[spanTarget.viewId];
	if (view === undefined) {
		throw new Error(
			`token annotation ${annotation.id} targets missing view ${spanTarget.viewId}`,
		);
	}
	if (spanTarget.span.unit !== "utf16-code-unit") {
		throw new Error(
			`token annotation ${annotation.id} uses unsupported ${spanTarget.span.unit} span unit for source text slicing`,
		);
	}
	return view.text.slice(spanTarget.span.start, spanTarget.span.end);
}

function getEntryTokenAnnotations(
	entry: TextCorpusEntry,
): readonly TextCorpusTokenAnnotation[] {
	const tokenLayer = findTokenLayer(
		entry.document,
		entry.tokenLayerId,
		entry.viewId,
	);
	...
	return Object.values(tokenLayer.annotations);
}

function getEntryTokenTexts(entry: TextCorpusEntry): readonly string[] {
	return getEntryTokenAnnotations(entry).map((annotation) =>
		resolveTokenText(entry.document, annotation),
	);
}

function tokenSpan(
	annotation: TextCorpusTokenAnnotation,
): { readonly startCU: number; readonly endCU: number } | undefined {
	const target = annotation.spans[0];
	if (target !== undefined && target.span.unit !== "utf16-code-unit") {
		throw new Error(
			`token annotation ${annotation.id} uses unsupported ${target.span.unit} span unit for source text slicing`,
		);
	}
	return target === undefined
		? undefined
		: { startCU: target.span.start, endCU: target.span.end };
}
...
function citationTextFromTokens(
	...
	const effectiveEnd = Math.max(
		effectiveStart,
		Math.min(tokenEnd, annotations.length),
	);
	const selected = annotations.slice(effectiveStart, effectiveEnd);
	const firstSpan =
		selected.length > 0
			? tokenSpan(selected[0] as TextCorpusTokenAnnotation)
			: undefined;
	const lastSpan =
		selected.length > 0
			? tokenSpan(selected[selected.length - 1] as TextCorpusTokenAnnotation)
			: undefined;
	const view = entry.document.views[entry.viewId];
	if (view !== undefined && firstSpan !== undefined && lastSpan !== undefined) {
		return {
			text: view.text.slice(firstSpan.startCU, lastSpan.endCU),
...
function normalizeMetadataFilters(
	...
	for (const [key, value] of Object.entries(filters).sort(([left], [right]) =>
		...
		if (!isNonEmptyString(key)) {
			throw new TypeError(
				"textcorpus metadata filter keys must be non-empty strings",
			);
		}
		if (typeof value === "string") {
			if (!isNonEmptyString(value)) {
				throw new TypeError(
					`metadata filter ${key} must be a non-empty string`,
				);
			}
		...
		if (
			...
			throw new TypeError(
				`metadata filter ${key} must be a non-empty string or string array`,
			);
		}
...
function selectTextCorpusEntries(
	...
	if (!isTextCorpusCollectionV1(collection)) {
		throw new TypeError(
			"textcorpus collection must satisfy TextCorpusCollectionV1",
		);
	}
	...
	const documents = entries.map(({ entry, tokens }) => ({
		...
		revision: textDocumentRevision(entry.document),
		viewId: entry.viewId,
	...
	return {
		...
		selection: {
			...
			tokenCount: entries.reduce(
				(sum, selected) => sum + selected.tokens.length,
				0,
			),
			documents,
...
function tokenCountMap(
	selectedEntries: readonly TextCorpusSelectedEntry[],
...
function documentFrequencyMapForTerms(
	selectedEntries: readonly TextCorpusSelectedEntry[],
): ReadonlyMap<string, number> {
...
function pmiLog2(
	...
	if (count <= 0 || termCount <= 0 || coTermCount <= 0 || totalPairs <= 0)
		return 0;
...
function validateQueries(queries: readonly TextCorpusQuery[]): void {
	...
	for (const query of queries) {
		...
		if (
			...
			!query.tokens.every((token) => typeof token === "string")
		) {
			throw new TypeError(
				`textcorpus query ${query.id} tokens must be a string array`,
			);
		}
	}
}

function smoothSklearnIdf(
...
function bm25OkapiParameters(formula: TextCorpusBm25FormulaId): {
	readonly k1: number;
	readonly b: number;
} {
...
function bm25OkapiScore(
	...
	idf: number,
	parameters: { readonly k1: number; readonly b: number } = bm25OkapiParameters(
...
function bm25fScore(weightedTermFrequency: number, idf: number): number {
	...
	const k1 = 1.2;
	return (
		idf * ((weightedTermFrequency * (k1 + 1)) / (weightedTermFrequency + k1))
	);
}

function isTermValueArray(
	value: unknown,
): value is readonly TextCorpusTermValue[] {
	return (
		Array.isArray(value) &&
		value.every(
			(entry) =>
...
function isTfidfFormula(value: unknown): value is TextCorpusTfidfFormulaId {
	return (
		value === textCorpusTfidfSklearnSmoothRawFormula ||
		value === textCorpusTfidfSklearnSmoothL2Formula
	);
}

function isBm25ScoringFormula(
	value: unknown,
): value is TextCorpusBm25FormulaId {
	return (
		value === textCorpusBm25OkapiFormula ||
		value === textCorpusBm25OkapiK1_1_2Formula
	);
}
...
function isFormulaTermValuesArray(
	value: unknown,
): value is readonly TextCorpusFormulaTermValues[] {
	return (
		Array.isArray(value) &&
		value.every(
			(entry) =>
...
function isDocumentScoreArray(
	value: unknown,
): value is readonly TextCorpusDocumentScore[] {
	return (
		Array.isArray(value) &&
		value.every(
			(entry) =>
...
function isFormulaDocumentScoresArray(
	value: unknown,
): value is readonly TextCorpusFormulaDocumentScores[] {
	return (
		Array.isArray(value) &&
		value.every(
			(entry) =>
...
function isRetrievalFormula(
	value: unknown,
): value is TextCorpusRetrievalFormulaId {
	return (
		value === textCorpusBm25OkapiFormula || value === textCorpusBm25fFormula
	);
}
...
function isRelevanceJudgment(
	value: unknown,
): value is TextCorpusRelevanceJudgment {
	return (
		isRecord(value) &&
...
function isRetrievalQrelsSource(
	value: unknown,
): value is TextCorpusRetrievalQrelsSource {
	return (
		isRecord(value) &&
...
function isRetrievalDocumentField(
	value: unknown,
): value is TextCorpusRetrievalDocumentField {
...
function isRetrievalDocument(
	value: unknown,
): value is TextCorpusRetrievalDocument {
	if (!isRecord(value)) return false;
	...
	return (
		...
		(value.fields === undefined ||
			(Array.isArray(value.fields) &&
				value.fields.every((field) => isRetrievalDocumentField(field)))) &&
		(value.metadata === undefined || isStringRecord(value.metadata))
	);
}

function isRetrievalFieldSpec(
	value: unknown,
): value is TextCorpusRetrievalNormalizedFieldSpec {
	if (!isRecord(value)) return false;
	return (
		isNonEmptyString(value.id) &&
...
function isTextCorpusRetrievalFieldWeightV1(
	value: unknown,
): value is TextCorpusRetrievalFieldWeightV1 {
	return (
		isRecord(value) &&
...
export function isTextCorpusRetrievalFieldWeightProfileV1(
	...
	return (
		...
		JSON.stringify(fieldIds) ===
			JSON.stringify([...fieldIds].sort(compareTerms))
	);
}
...
function normalizeRetrievalFieldWeightEntries(
	...
	if (!isNumberRecord(fields)) {
		throw new TypeError(
			"textcorpus retrieval field weights must be a string-to-non-negative-number record",
		);
	}
	...
	if (entries.length === 0) {
		throw new TypeError(
			"textcorpus retrieval field weights must include at least one field",
		);
	}
	return entries;
}
...
export function createTextCorpusRetrievalFieldWeightProfile(
	...
	if (!isRecord(options) || !isNonEmptyString(options.profileId)) {
		throw new TypeError(
			"textcorpus retrieval field weight profile id must be a non-empty string",
		);
	}
	const profile = {
...
function validateEntry(entry: TextCorpusEntry): void {
	...
	if (!isTextDocument(entry.document)) {
		throw new TypeError(
			`entry ${entry.id} must carry a valid TextDocument document`,
		);
	}
	...
	if (!isNonEmptyString(entry.tokenLayerId)) {
		throw new TypeError(
			`entry ${entry.id} tokenLayerId must be a non-empty string`,
		);
	}
	if (entry.metadata !== undefined && !isStringRecord(entry.metadata)) {
		throw new TypeError(
			`entry ${entry.id} metadata must be a string-to-string record`,
		);
	}
	if (!findView(entry.document, entry.viewId)) {
		throw new Error(
			`entry ${entry.id} references missing view ${entry.viewId}`,
		);
	}
...
export function isTextCorpusCollectionV1(
	value: unknown,
): value is TextCorpusCollectionV1 {
	return (
		isRecord(value) &&
...
function isTextCorpusEvidenceClass(
	value: unknown,
): value is TextCorpusEvidenceClass {
	return value === textCorpusEvidenceClassE2;
}

function isNormalizedMetadataFilters(
	value: unknown,
): value is TextCorpusNormalizedMetadataFilters {
	return (
		isRecord(value) &&
		Object.entries(value).every(
			([key, entryValue]) =>
...
function isTextCorpusSelectionDocumentRefV1(
	value: unknown,
): value is TextCorpusSelectionDocumentRefV1 {
	return (
		isRecord(value) &&
		isNonEmptyString(value.id) &&
...
export function isTextCorpusSelectionProvenanceV1(
	value: unknown,
): value is TextCorpusSelectionProvenanceV1 {
	if (
		!(
			isRecord(value) &&
			...
			value.documents.every((entry) =>
				isTextCorpusSelectionDocumentRefV1(entry),
			) &&
			(value.metadataFilters === undefined ||
				isNormalizedMetadataFilters(value.metadataFilters))
		)
	) {
		return false;
	}
	const documents =
		value.documents as readonly TextCorpusSelectionDocumentRefV1[];
	const documentOrder = value.documentOrder as readonly string[];
	return (
		documents.map((entry) => entry.id).join("\u001f") ===
			documentOrder.join("\u001f") &&
		documents.reduce((sum, entry) => sum + entry.tokenCount, 0) ===
			value.tokenCount
	);
}
...
function selectionMatchesCorpus(
	...
	return (
		isTextCorpusSelectionProvenanceV1(selection) &&
		selection.corpusId === corpusId
	);
}

function isTextCorpusConcordanceRowV1(
	value: unknown,
): value is TextCorpusConcordanceRowV1 {
	return (
		isRecord(value) &&
		isNonEmptyString(value.docId) &&
		isNonEmptyString(value.documentId) &&
...
export function isTextCorpusConcordanceResultV1(
	value: unknown,
): value is TextCorpusConcordanceResultV1 {
	return (
		isRecord(value) &&
		value.schemaVersion === textCorpusAnalysisSchemaVersion &&
		isNonEmptyString(value.corpusId) &&
...
export function isTextCorpusFrequencyResultV1(
	value: unknown,
): value is TextCorpusFrequencyResultV1 {
	return (
		isRecord(value) &&
		value.schemaVersion === textCorpusAnalysisSchemaVersion &&
		isNonEmptyString(value.corpusId) &&
...
export function isTextCorpusNgramResultV1(
	value: unknown,
): value is TextCorpusNgramResultV1 {
	return (
		isRecord(value) &&
		value.schemaVersion === textCorpusAnalysisSchemaVersion &&
		isNonEmptyString(value.corpusId) &&
...
function isCooccurrenceRow(
	value: unknown,
): value is TextCorpusCooccurrenceRowV1 {
	return (
		isRecord(value) &&
		typeof value.term === "string" &&
...
export function isTextCorpusCooccurrenceResultV1(
	value: unknown,
): value is TextCorpusCooccurrenceResultV1 {
	return (
		isRecord(value) &&
		value.schemaVersion === textCorpusAnalysisSchemaVersion &&
		isNonEmptyString(value.corpusId) &&
...
export function isTextCorpusCollocateResultV1(
	value: unknown,
): value is TextCorpusCollocateResultV1 {
	return (
		isRecord(value) &&
		value.schemaVersion === textCorpusAnalysisSchemaVersion &&
		isNonEmptyString(value.corpusId) &&
...
function isPairwiseRelationLabel(
	value: unknown,
): value is TextCorpusPairwiseRelationLabel {
	return (
		value === "exact-duplicate" ||
		value === "near-duplicate" ||
		value === "shared-reuse"
	);
}

function isPairwiseRelationRow(
	value: unknown,
): value is TextCorpusPairwiseRelationRowV1 {
	return (
		isRecord(value) &&
...
export function isTextCorpusPairwiseRelationResultV1(
	value: unknown,
): value is TextCorpusPairwiseRelationResultV1 {
	return (
		isRecord(value) &&
		value.schemaVersion === textCorpusAnalysisSchemaVersion &&
		isNonEmptyString(value.corpusId) &&
...
export function isTextCorpusFingerprintIndex(
	...
	return (
		...
		Object.values(value.docFingerprints).every(
			(entry) =>
				Array.isArray(entry) && entry.every((hash) => isNonEmptyString(hash)),
		) &&
		isRecord(value.index) &&
		Object.values(value.index).every(
			(entry) =>
				Array.isArray(entry) && entry.every((docId) => isNonEmptyString(docId)),
		) &&
		(value.truncated === undefined || typeof value.truncated === "boolean")
	);
}

export function isTextCorpusScoringResultV1(
	value: unknown,
): value is TextCorpusScoringResultV1 {
	return (
		isRecord(value) &&
		...
		value.documents.every(
			(entry) =>
				...
				(entry.tfidfVariants === undefined ||
					isFormulaTermValuesArray(entry.tfidfVariants)),
		) &&
		...
		value.queries.every(
			(entry) =>
				...
				(entry.bm25Variants === undefined ||
					isFormulaDocumentScoresArray(entry.bm25Variants)),
		)
	);
}

export function isTextCorpusParsedQuery(
	value: unknown,
): value is TextCorpusParsedQuery {
	return (
		isRecord(value) &&
		isNonEmptyString(value.id) &&
			...
			(clause) =>
				...
				(clause.terms === undefined ||
					(Array.isArray(clause.terms) &&
						clause.terms.every((term) => typeof term === "string"))) &&
		...
		(value.expression === undefined ||
			isTextCorpusQueryExpression(value.expression)) &&
		(value.syntax === undefined ||
			value.syntax === "terms" ||
			value.syntax === "boolean")
	);
}

function isTextCorpusQueryExpression(
	value: unknown,
): value is TextCorpusQueryExpression {
	if (!isRecord(value)) return false;
...
export function isTextCorpusRetrievalIndexV1(
	value: unknown,
): value is TextCorpusRetrievalIndexV1 {
	return (
		isRecord(value) &&
		value.schemaVersion === textCorpusRetrievalSchemaVersion &&
		isNonEmptyString(value.corpusId) &&
		...
		Object.values(value.invertedIndex).every((entry) =>
			isPostingArray(entry),
		) &&
		(value.fieldOrder === undefined ||
			(Array.isArray(value.fieldOrder) &&
				value.fieldOrder.every((entry) => isNonEmptyString(entry)))) &&
		(value.fieldSpecs === undefined ||
			(Array.isArray(value.fieldSpecs) &&
				value.fieldSpecs.every((entry) => isRetrievalFieldSpec(entry)))) &&
		(value.fieldAverageLengths === undefined ||
			isNumberRecord(value.fieldAverageLengths)) &&
		(value.fieldInvertedIndex === undefined ||
			isFieldInvertedIndex(value.fieldInvertedIndex))
	);
}

export function isTextCorpusRetrievalResultV1(
	value: unknown,
): value is TextCorpusRetrievalResultV1 {
	return (
		isRecord(value) &&
		value.schemaVersion === textCorpusRetrievalSchemaVersion &&
		isNonEmptyString(value.corpusId) &&
...
export function isTextCorpusRetrievalQrelsV1(
	value: unknown,
): value is TextCorpusRetrievalQrelsV1 {
	return (
		isRecord(value) &&
...
function isRetrievalQueryEvaluation(
	value: unknown,
): value is TextCorpusRetrievalQueryEvaluation {
	return (
		isRecord(value) &&
		isNonEmptyString(value.queryId) &&
...
export function isTextCorpusRetrievalCalibrationReportV1(
	...
	if (
		...
		!value.candidateOrder.every((candidateId) =>
			isNonEmptyString(candidateId),
		) ||
		!Array.isArray(value.candidates) ||
		value.candidates.length === 0 ||
		!value.candidates.every((candidate) =>
			isTextCorpusRetrievalCalibrationCandidateV1(candidate),
		)
	) {
		return false;
	}
	...
	return (
		...
		value.candidates.some(
			(candidate) =>
				candidate.selected &&
				candidate.candidateId === value.selectedCandidateId,
		) &&
		value.candidates.every((candidate) => candidate.formula === value.formula)
	);
}

function isTextCorpusCitationWindowV1(
	value: unknown,
): value is TextCorpusCitationWindowV1 {
	return (
		isRecord(value) &&
		value.schemaVersion === textCorpusRetrievalSchemaVersion &&
		isNonEmptyString(value.corpusId) &&
...
export function isTextCorpusCitationWindowSetV1(
	value: unknown,
): value is TextCorpusCitationWindowSetV1 {
	return (
		isRecord(value) &&
		value.schemaVersion === textCorpusRetrievalSchemaVersion &&
		isNonEmptyString(value.corpusId) &&
...
function isTextCorpusQuoteGroundingMatchV1(
	value: unknown,
): value is TextCorpusQuoteGroundingMatchV1 {
	return (
		isRecord(value) &&
		isNonEmptyString(value.docId) &&
		isNonEmptyString(value.documentId) &&
...
export function isTextCorpusQuoteGroundingResultV1(
	value: unknown,
): value is TextCorpusQuoteGroundingResultV1 {
	return (
		isRecord(value) &&
		value.schemaVersion === textCorpusRetrievalSchemaVersion &&
		isNonEmptyString(value.corpusId) &&
...
export function createTextCorpusCollection(
	...
	if (!isNonEmptyString(options.corpusId)) {
		throw new TypeError(
			"textcorpus collection corpusId must be a non-empty string",
		);
	}
	...
	for (const entry of normalizedEntries) {
		...
		if (seenDocumentIds.has(entry.document.id)) {
			throw new Error(`duplicate textcorpus documentId: ${entry.document.id}`);
		}
...
export function computeTextCorpusConcordance(
	...
	if (!isNonEmptyString(options.query)) {
		throw new TypeError(
			"textcorpus concordance query must be a non-empty string",
		);
	}
	const window = nonNegativeInteger(
		options.window ?? 5,
		"textcorpus concordance window",
	);
	const { entries, selection } = selectTextCorpusEntries(
		collection,
...
export function computeTextCorpusCooccurrences(
	...
	const window = positiveInteger(
		options.window ?? 5,
		"textcorpus co-occurrence window",
	);
	const { entries, selection } = selectTextCorpusEntries(
		collection,
		options.metadataFilters,
	...
	const rows = [...pairCounts.entries()]
		...
		.sort(
			(left, right) =>
				...
				left.coTerm.localeCompare(right.coTerm),
		);
	return {
		schemaVersion: textCorpusAnalysisSchemaVersion,
		corpusId: collection.corpusId,
...
export function computeTextCorpusPairwiseRelations(
	...
	const shingleSize = positiveInteger(
		options.shingleSize,
		"textcorpus pairwise shingle size",
	);
	const windowSize = positiveInteger(
		options.windowSize,
		"textcorpus pairwise window size",
	);
	const nearDuplicateThreshold = options.nearDuplicateThreshold ?? 0.8;
	if (
		!Number.isFinite(nearDuplicateThreshold) ||
		nearDuplicateThreshold < 0 ||
		nearDuplicateThreshold > 1
	) {
		throw new TypeError(
			"textcorpus near-duplicate threshold must be between 0 and 1",
		);
	}
	const { entries, selection } = selectTextCorpusEntries(
		collection,
	...
	for (let leftIndex = 0; leftIndex < documentOrder.length; leftIndex += 1) {
		...
		const leftFingerprints = new Set(
			fingerprintIndex.docFingerprints[leftDocId] ?? [],
		);
		for (
			let rightIndex = leftIndex + 1;
			rightIndex < documentOrder.length;
			rightIndex += 1
		) {
			const rightDocId = documentOrder[rightIndex] ?? "";
			const rightFingerprints = new Set(
				fingerprintIndex.docFingerprints[rightDocId] ?? [],
			);
			const shared = [...leftFingerprints].filter((fingerprint) =>
				rightFingerprints.has(fingerprint),
			);
			if (shared.length === 0) continue;
			const unionCount = new Set([...leftFingerprints, ...rightFingerprints])
				.size;
			const jaccard = unionCount === 0 ? 0 : shared.length / unionCount;
			const relation: TextCorpusPairwiseRelationLabel =
				jaccard === 1
					? "exact-duplicate"
					: jaccard >= nearDuplicateThreshold
						? "near-duplicate"
						: "shared-reuse";
			rows.push({
				leftDocId,
	...
	rows.sort(
		(left, right) =>
			left.leftDocId.localeCompare(right.leftDocId) ||
			left.rightDocId.localeCompare(right.rightDocId),
	);
	return {
		schemaVersion: textCorpusAnalysisSchemaVersion,
		corpusId: collection.corpusId,
...
export function buildTextCorpusFingerprintIndex(
	...
	const maxFingerprintsPerDoc =
		options.maxFingerprintsPerDoc ?? Number.POSITIVE_INFINITY;
	...
	for (const entry of collection.entries) {
		...
		for (
			let tokenIndex = 0;
			tokenIndex <= tokenTexts.length - shingleSize;
			tokenIndex += 1
		) {
			const shingleText = tokenTexts
				.slice(tokenIndex, tokenIndex + shingleSize)
				.join("\u001f");
			shingleHashes.push(
				stableHash64(shingleText, { algorithm: hashAlgorithm }),
			);
		}
...
function uniqueTfidfFormulas(
	formulas: readonly TextCorpusTfidfFormulaId[] | undefined,
): readonly TextCorpusTfidfFormulaId[] {
	...
	for (const formula of requested) {
		if (!isTfidfFormula(formula))
			throw new TypeError(
				`unsupported textcorpus TF-IDF formula: ${String(formula)}`,
			);
		if (!output.includes(formula)) output.push(formula);
	}
	if (!output.includes(textCorpusTfidfSklearnSmoothRawFormula))
		output.unshift(textCorpusTfidfSklearnSmoothRawFormula);
	return output;
}

function uniqueBm25Formulas(
	formulas: readonly TextCorpusBm25FormulaId[] | undefined,
): readonly TextCorpusBm25FormulaId[] {
	const requested = formulas ?? [textCorpusBm25OkapiFormula];
	const output: TextCorpusBm25FormulaId[] = [];
	for (const formula of requested) {
		if (!isBm25ScoringFormula(formula))
			throw new TypeError(
				`unsupported textcorpus BM25 formula: ${String(formula)}`,
			);
		if (!output.includes(formula)) output.push(formula);
	}
	if (!output.includes(textCorpusBm25OkapiFormula))
		output.unshift(textCorpusBm25OkapiFormula);
	return output;
}

function l2NormalizeTermValues(
	values: readonly TextCorpusTermValue[],
): readonly TextCorpusTermValue[] {
	const norm = Math.sqrt(
		values.reduce((sum, entry) => sum + entry.value ** 2, 0),
	);
	if (norm === 0) return [];
...
export function computeTextCorpusScoring(
	...
	const averageDocumentLength =
		tokenLists.reduce((sum, tokens) => sum + tokens.length, 0) /
	...
	const documents = entries.map(({ entry }, index) => {
		...
		const tfidfVariants = tfidfFormulas.map((formula) => ({
			formula,
			values:
				formula === textCorpusTfidfSklearnSmoothL2Formula
					? l2NormalizeTermValues(tfidf)
					: tfidf,
		}));
		return {
			id: entry.id,
...
function normalizeQueryField(raw: string): string {
	...
	if (!isNonEmptyString(field))
		throw querySyntaxError("field name must contain a token");
	return field;
}
	...
	while (cursor < raw.length) {
		...
		if (char === '"') {
			...
			if (raw[cursor] === "~") {
				...
				while (cursor < raw.length && /[0-9]/u.test(raw[cursor] as string))
					cursor += 1;
				if (cursor === numberStart)
					throw querySyntaxError(
						"proximity phrase must include an integer distance",
					);
...
function readQueryWord(
	raw: string,
	start: number,
): { readonly value: string; readonly end: number } {
	let cursor = start;
	while (cursor < raw.length && !/\s|\(|\)|"/u.test(raw[cursor] as string))
		cursor += 1;
	...
	while (cursor < raw.length) {
		...
		if (raw[cursor] === '"') {
			...
			const clause = makeQueryClause(quoted.value, {
				...
				...(quoted.proximity === undefined
					? {}
					: { proximity: quoted.proximity }),
			});
			if (clause !== undefined) tokens.push({ type: "atom", clause });
			cursor = quoted.end;
			continue;
...
function queryExpressionFromClause(
	clause: TextCorpusParsedQueryClause,
): TextCorpusQueryExpression {
...
function collectExpressionClauses(
	expression: TextCorpusQueryExpression,
	output: TextCorpusParsedQueryClause[] = [],
): TextCorpusParsedQueryClause[] {
	...
	for (const child of expression.children ?? [])
		collectExpressionClauses(child, output);
	return output;
}

function parseQueryExpression(
	tokens: readonly QueryToken[],
): TextCorpusQueryExpression | undefined {
	let cursor = 0;
	...
	function parsePrimary(): TextCorpusQueryExpression {
		...
		if (token.type === "unary") {
			...
			if (child.kind === "clause" && child.clause !== undefined) {
				return queryExpressionFromClause(
					applyQueryOperator(child.clause, operator),
				);
			}
		...
		if (token.type === "paren" && token.value === "(") {
			...
			if (close?.type !== "paren" || close.value !== ")")
				throw querySyntaxError("unclosed parenthesized expression");
			return expression;
		}
	...
	function parseAnd(): TextCorpusQueryExpression {
		...
		while (
			peek()?.type === "operator" &&
			(peek() as { readonly value?: string }).value === "AND"
		) {
			consume();
	...
	function parseOr(): TextCorpusQueryExpression {
		...
		while (
			(peek()?.type === "operator" &&
				(peek() as { readonly value?: string }).value === "OR") ||
			nextTokenStartsImplicitOr()
		) {
	...
	if (cursor !== tokens.length)
		throw querySyntaxError("unexpected trailing query token");
	return expression;
}
...
function parseQueryClauses(raw: string): {
	...
	const clauses =
		expression === undefined ? [] : collectExpressionClauses(expression);
	return {
		clauses,
...
export function parseTextCorpusQuery(
	...
	if (!isNonEmptyString(id)) {
		throw new TypeError(
			"textcorpus parsed query id must be a non-empty string",
		);
	}
	return {
		id,
...
export function buildTextCorpusRetrievalIndex(
	...
	if (!isRetrievalFormula(formula)) {
		throw new TypeError(
			"textcorpus retrieval formula must be a known formula id",
		);
	}
	const fieldSpecs =
		formula === textCorpusBm25fFormula
			? normalizeRetrievalFieldSpecs(options.fields)
			: [];
	const { entries, selection } = selectTextCorpusEntries(collection);

	const documents = entries.map(({ entry, tokens }) => {
		const fields = fieldSpecs.map((fieldSpec) => {
			const fieldTokensForEntry = retrievalFieldTokens(
				entry,
				tokens,
				fieldSpec,
			);
	...
	const fieldPostingsByField = new Map<
		string,
	...
	const termOrder = [
		...new Set([...postingsByTerm.keys(), ...fieldTermSet]),
	].sort(compareTerms);
	...
	const fieldInvertedIndex: Record<
		string,
		Readonly<Record<string, readonly TextCorpusPosting[]>>
	> = {};
	for (const fieldSpec of fieldSpecs) {
		const fieldIndex =
			fieldPostingsByField.get(fieldSpec.id) ??
		...
		for (const term of [...fieldIndex.keys()].sort(compareTerms)) {
			...
			normalizedFieldIndex[term] = [...postings.entries()]
				.sort(([leftDocId], [rightDocId]) =>
					leftDocId.localeCompare(rightDocId),
				)
				.map(([docId, positions]) => ({ docId, positions }));
		}
...
function normalizeRetrievalFieldSpecs(
	...
	return rawSpecs.map((spec) => {
		if (!isNonEmptyString(spec.id)) {
			throw new TypeError(
				"textcorpus retrieval field id must be a non-empty string",
			);
		}
		if (seen.has(spec.id)) {
		...
		if (spec.source !== "tokens" && spec.source !== "metadata") {
			throw new TypeError(
				`textcorpus retrieval field ${spec.id} source must be tokens or metadata`,
			);
		}
		const metadataKey =
			spec.source === "metadata" ? (spec.metadataKey ?? spec.id) : undefined;
		if (spec.source === "metadata" && !isNonEmptyString(metadataKey)) {
			throw new TypeError(
				`textcorpus retrieval metadata field ${spec.id} must name a metadata key`,
			);
		}
		const weight = spec.weight ?? 1;
		if (!Number.isFinite(weight) || weight <= 0) {
			throw new TypeError(
				`textcorpus retrieval field ${spec.id} weight must be a positive number`,
			);
		}
		const b = spec.b ?? 0.75;
		if (!Number.isFinite(b) || b < 0 || b > 1) {
			throw new TypeError(
				`textcorpus retrieval field ${spec.id} b must be between 0 and 1`,
			);
		}
		return {
			id: spec.id,
...
function validateRetrievalQrels(qrels: TextCorpusRetrievalQrelsV1): void {
	if (!isTextCorpusRetrievalQrelsV1(qrels)) {
		throw new TypeError(
			"textcorpus retrieval qrels must satisfy TextCorpusRetrievalQrelsV1",
		);
	}
	...
	for (const judgment of qrels.judgments) {
		if (seenQueries.has(judgment.queryId)) {
			throw new Error(
				`duplicate textcorpus qrels query id: ${judgment.queryId}`,
			);
		}
		...
		for (const rating of judgment.ratings) {
			if (seenDocs.has(rating.docId)) {
				throw new Error(
					`duplicate textcorpus qrels doc id: ${judgment.queryId}/${rating.docId}`,
				);
			}
...
function evaluateQueryRetrieval(
	...
	const resultForQuery = result.results.find(
		(entry) => entry.query.id === judgment.queryId,
	);
	const retrieved = (resultForQuery?.hits ?? [])
		.slice(0, k)
		.map((hit) => hit.docId);
	const gradeByDoc = new Map(
		judgment.ratings.map((rating) => [rating.docId, rating.grade]),
	);
	...
	const relevantRetrievedCount = retrieved.filter((docId) =>
		relevantSet.has(docId),
	).length;
	const precisionAtK = relevantRetrievedCount / k;
	const recallAtK =
		relevant.length === 0 ? 0 : relevantRetrievedCount / relevant.length;
	const firstRelevantIndex = retrieved.findIndex((docId) =>
		relevantSet.has(docId),
	);
	...
	const ndcgAtK =
		idealDcg === 0 ? 0 : discountedGainAtK(actualGrades, k) / idealDcg;
	return {
...
function scoringTokens(query: TextCorpusParsedQuery): readonly string[] {
	return query.clauses
		.filter(
			(clause) => clause.operator !== "must-not" && clause.field === undefined,
		)
...
function positionsForTerm(
	index: TextCorpusRetrievalIndexV1,
	term: string,
	docId: string,
): readonly number[] {
	return (
		index.invertedIndex[term]?.find((posting) => posting.docId === docId)
			?.positions ?? []
	);
}
...
function positionsForFieldTerm(
	...
	return (
		index.fieldInvertedIndex?.[field]?.[term]?.find(
	...
	for (const firstPosition of termPositions[0] ?? []) {
		...
		for (let termIndex = 1; termIndex < termPositions.length; termIndex += 1) {
			const next = (termPositions[termIndex] ?? []).find(
				(position) =>
					position > previous && position - previous - 1 <= proximity,
			);
			if (next === undefined) {
				matched = false;
				break;
...
function documentTokensForClause(
	...
	if (clause.field !== undefined) {
		if ((index.fieldOrder ?? []).includes(clause.field))
			return fieldTokens(document, clause.field);
...
function documentMatchesExpression(
	...
	if (expression.kind === "not") {
		return !(expression.children ?? []).some((child) =>
			documentMatchesExpression(index, document, child),
		);
	}
...
function documentMatchesQuery(
	...
	const mustClauses = query.clauses.filter(
		(clause) => clause.operator === "must",
	);
	if (
		!mustClauses.every((clause) =>
			documentMatchesClause(index, document, clause),
		)
	)
		return false;
	const prohibitedClauses = query.clauses.filter(
		(clause) => clause.operator === "must-not",
	);
	if (
		prohibitedClauses.some((clause) =>
			documentMatchesClause(index, document, clause),
		)
	)
		return false;
	const shouldClauses = query.clauses.filter(
		(clause) => clause.operator === "should",
	);
	const shouldFieldClauses = shouldClauses.filter(
		(clause) => clause.field !== undefined,
	);
	if (
		!shouldFieldClauses.every((clause) =>
			documentMatchesClause(index, document, clause),
		)
	) {
		return false;
	}
	const shouldTermClauses = shouldClauses.filter(
		(clause) => clause.field === undefined,
	);
	if (mustClauses.length === 0 && shouldTermClauses.length > 0) {
		return shouldTermClauses.some((clause) =>
			documentMatchesClause(index, document, clause),
		);
	}
	return true;
...
function createRetrievalSnippet(
	...
	const matchedPositions = document.tokens.flatMap((token, position) =>
		queryTokenSet.has(token) ? [position] : [],
	);
	if (matchedPositions.length === 0) return undefined;
	const effectiveWindow = Math.max(0, Math.floor(windowSize));
	const tokenStart = Math.max(
		0,
		Math.min(...matchedPositions) - effectiveWindow,
	);
	const tokenEnd = Math.min(
		document.tokens.length,
		Math.max(...matchedPositions) + effectiveWindow + 1,
	);
	...
	return {
		...
		tokenEnd,
		highlightedTerms: [
			...new Set(snippetTokens.filter((token) => queryTokenSet.has(token))),
		].sort(compareTerms),
	};
}
...
function scoreRetrievalDocument(
	...
	const explain = uniqueQueryTokens(clauses.map((clause) => clause.term)).map(
		(term) => {
			...
			const contribution = bm25OkapiScore(
				tf,
				document.length,
...
function fieldDocumentFrequency(
	...
	if (field !== undefined)
		return index.fieldInvertedIndex?.[field]?.[term]?.length ?? 0;
...
function scoreBm25fRetrievalClause(
	...
	const fieldIds =
		clause.field !== undefined ? [clause.field] : (index.fieldOrder ?? []);
	const fieldContributions = fieldIds.flatMap(
		(fieldId): readonly TextCorpusRetrievalFieldContribution[] => {
			...
			const tf = positionsForFieldTerm(
				index,
				fieldId,
				clause.term,
				document.id,
				...
				return [
					{
						...
						...(fieldWeights === undefined
							? {}
							: { baseWeight: fieldSpec.weight, queryWeight }),
						normalizedTf: 0,
					},
				];
			}
			const denominator =
				1 - fieldSpec.b + fieldSpec.b * (length / averageLength);
			const normalizedTf =
				denominator === 0 ? 0 : effectiveWeight * (tf / denominator);
			return [
				{
					field: fieldId,
					tf,
	...
	const weightedTf = fieldContributions.reduce(
		(sum, entry) => sum + entry.normalizedTf,
		0,
	);
...
function searchSingleTextCorpusRetrievalQuery(
	...
	const hits = index.documentOrder.flatMap((docId) => {
		...
		const snippet = createRetrievalSnippet(
			document,
			tokensForSnippet,
...
function normalizeSearchFieldWeightProfile(
	...
	if (
		...
		options.fieldWeightProfile !== undefined
	) {
		throw new TypeError(
			"textcorpus retrieval field weights must be provided directly or as a profile, not both",
		);
	}
	const profile =
		options.fieldWeightProfile ??
		(options.fieldWeights === undefined
			? undefined
	...
	if (index.formula !== textCorpusBm25fFormula) {
		throw new TypeError(
			"textcorpus retrieval field weights require a BM25F retrieval index",
		);
	}
	if (!isTextCorpusRetrievalFieldWeightProfileV1(profile)) {
		throw new TypeError("textcorpus retrieval field weight profile is invalid");
	...
	for (const field of profile.fields) {
		if (!fieldSet.has(field.field)) {
			throw new Error(
				`textcorpus retrieval field weight references unknown field: ${field.field}`,
			);
		}
	}
	return profile;
...
function normalizeRetrievalSearchOptions(
	...
	return {
		...
		...(options.includeZeroScores === undefined
			? {}
			: { includeZeroScores: options.includeZeroScores }),
		...(fieldWeightProfile === undefined
			? {}
			: {
					fieldWeightProfile,
					fieldWeights: Object.fromEntries(
						fieldWeightProfile.fields.map((field) => [
							field.field,
							field.weight,
						]),
					),
...
export function* iterateTextCorpusRetrievalResults(
	...
	if (!isTextCorpusRetrievalIndexV1(index)) {
		throw new TypeError(
			"textcorpus retrieval index must satisfy TextCorpusRetrievalIndexV1",
		);
	}
	...
	for (const query of queries) {
		if (!isTextCorpusParsedQuery(query)) {
			...
			throw new TypeError(
				"textcorpus retrieval queries must be parsed query objects",
			);
		}
		yield searchSingleTextCorpusRetrievalQuery(
			index,
			query,
			normalizedOptions,
			documentById,
...
export function searchTextCorpusRetrievalIndex(
	...
	const normalizedOptions = normalizeRetrievalSearchOptions(index, options);
	const results = [
		...iterateTextCorpusRetrievalResults(index, queries, options),
	];
	return {
		schemaVersion: textCorpusRetrievalSchemaVersion,
...
export function evaluateTextCorpusRetrieval(
	...
	if (!isTextCorpusRetrievalResultV1(result)) {
		throw new TypeError(
			"textcorpus retrieval result must satisfy TextCorpusRetrievalResultV1",
		);
	}
	validateRetrievalQrels(qrels);
	if (result.corpusId !== qrels.corpusId) {
		throw new Error(
			`textcorpus retrieval qrels corpus mismatch: ${qrels.corpusId} != ${result.corpusId}`,
		);
	}
	const k = Math.max(1, Math.floor(options.k ?? 10));
	const relevantGradeThreshold = Math.max(
		1,
		Math.floor(options.relevantGradeThreshold ?? 1),
	);
	const tolerance = options.tolerance ?? 1e-12;
	if (!Number.isFinite(tolerance) || tolerance < 0) {
		throw new TypeError(
			"textcorpus retrieval evaluation tolerance must be a non-negative finite number",
		);
	}
	const queries = [...qrels.judgments]
		.sort((left, right) => left.queryId.localeCompare(right.queryId))
		.map((judgment) =>
			evaluateQueryRetrieval(result, judgment, k, relevantGradeThreshold),
		);
	return {
...
function validateRetrievalCalibrationCandidateInput(
	...
	if (!isRecord(candidate) || !isNonEmptyString(candidate.candidateId)) {
		throw new TypeError(
			"textcorpus retrieval calibration candidate id must be a non-empty string",
		);
	}
	if (!isTextCorpusRetrievalEvaluationResultV1(candidate.evaluation)) {
		throw new TypeError(
			"textcorpus retrieval calibration candidate evaluation is invalid",
		);
	}
	if (
		...
		throw new TypeError(
			"textcorpus retrieval calibration field weight profile is invalid",
		);
	}
	if (
		candidate.fieldWeightProfile !== undefined &&
...
function assertComparableRetrievalCalibrationEvaluation(
	...
	if (candidate.corpusId !== reference.corpusId) {
		throw new Error(
			`textcorpus retrieval calibration corpus mismatch for candidate ${candidateId}`,
		);
	}
	if (candidate.formula !== reference.formula) {
		throw new Error(
			`textcorpus retrieval calibration formula mismatch for candidate ${candidateId}`,
		);
	}
	if (candidate.k !== reference.k) {
		throw new Error(
			`textcorpus retrieval calibration k mismatch for candidate ${candidateId}`,
		);
	}
	...
	if (candidate.tolerance !== reference.tolerance) {
		throw new Error(
			`textcorpus retrieval calibration evaluation tolerance mismatch for candidate ${candidateId}`,
		);
	}
	if (
		JSON.stringify(candidate.selection) !== JSON.stringify(reference.selection)
	) {
		throw new Error(
			`textcorpus retrieval calibration selection mismatch for candidate ${candidateId}`,
		);
	}
}
...
export function createTextCorpusRetrievalCalibrationReport(
	...
	if (!Array.isArray(candidates) || candidates.length === 0) {
		throw new TypeError(
			"textcorpus retrieval calibration candidates must be a non-empty array",
		);
	}
	for (const candidate of candidates)
		validateRetrievalCalibrationCandidateInput(candidate);
	const normalizedCandidates = [...candidates].sort((left, right) =>
		left.candidateId.localeCompare(right.candidateId),
	);
	const candidateIds = normalizedCandidates.map(
		(candidate) => candidate.candidateId,
	);
	...
	if (referenceEvaluation === undefined) {
		throw new TypeError(
			"textcorpus retrieval calibration candidates must include an evaluation",
		);
	}
	...
	if (!isTextCorpusRetrievalCalibrationMetric(optimizeMetric)) {
		throw new TypeError(
			"textcorpus retrieval calibration optimize metric is invalid",
		);
	}
	...
	if (
		...
		throw new TypeError(
			"textcorpus retrieval calibration tolerance must be a non-negative finite number",
		);
	}
	const baselineCandidateId =
		options.baselineCandidateId ??
		(candidateIds.includes("baseline")
			? "baseline"
			: (candidateIds[0] as string));
	if (!candidateIds.includes(baselineCandidateId)) {
		throw new Error(
			`textcorpus retrieval calibration baseline candidate is missing: ${baselineCandidateId}`,
		);
	}
	const reportId =
		options.reportId ??
		`textcorpus.retrieval-calibration:${referenceEvaluation.corpusId}`;
	if (!isNonEmptyString(reportId)) {
		throw new TypeError(
			"textcorpus retrieval calibration report id must be a non-empty string",
		);
	}
	...
	const ranked = normalizedCandidates.sort((left, right) => {
		const leftScore = retrievalEvaluationMetric(
			left.evaluation.summary,
			optimizeMetric,
		);
		const rightScore = retrievalEvaluationMetric(
	...
	if (selectedCandidate === undefined) {
		throw new TypeError(
			"textcorpus retrieval calibration candidates must include a selected candidate",
		);
	}
	const selectedMetricScore = retrievalEvaluationMetric(
		selectedCandidate.evaluation.summary,
		optimizeMetric,
	);
	const reportCandidates = ranked.map(
		(candidate, index): TextCorpusRetrievalCalibrationCandidateV1 => {
			const metricScore = retrievalEvaluationMetric(
			...
			return {
				...
				...(candidate.fieldWeightProfile === undefined
					? {}
					: { fieldWeightProfile: candidate.fieldWeightProfile }),
				...
				metricScore,
				withinToleranceOfSelected:
					Math.abs(selectedMetricScore - metricScore) <= tolerance,
			};
		},
	);
	const report = {
...
export function calibrateTextCorpusRetrievalFieldWeightProfiles(
	...
	if (index.formula !== textCorpusBm25fFormula) {
		throw new TypeError(
			"textcorpus retrieval field-weight calibration requires a BM25F retrieval index",
		);
	}
	if (
		!Array.isArray(queries) ||
		!queries.every((query) => isTextCorpusParsedQuery(query))
	) {
		throw new TypeError(
			"textcorpus retrieval calibration queries must be parsed textcorpus queries",
		);
	}
	validateRetrievalQrels(qrels);
	if (
		...
		!profiles.every((profile) =>
			isTextCorpusRetrievalFieldWeightProfileV1(profile),
		)
	) {
		throw new TypeError(
			"textcorpus retrieval calibration profiles must satisfy TextCorpusRetrievalFieldWeightProfileV1",
		);
	}
	const evaluationK = Math.max(1, Math.floor(options.k ?? 10));
	const searchTopK = Math.max(
		evaluationK,
		Math.max(1, Math.floor(options.searchTopK ?? evaluationK)),
	);
	...
	if (options.includeBaseline !== false) {
		const baselineResult = searchTextCorpusRetrievalIndex(index, queries, {
			topK: searchTopK,
			...(options.snippetWindow === undefined
				? {}
				: { snippetWindow: options.snippetWindow }),
		});
		candidates.push({
			candidateId: options.baselineCandidateId ?? "baseline",
			evaluation: evaluateTextCorpusRetrieval(
				baselineResult,
				qrels,
				evaluationOptions,
			),
...
function normalizeRetrievalFieldWeightLearningSearchSpace(
	...
	if (!Array.isArray(searchSpace) || searchSpace.length === 0) {
		throw new TypeError(
			"textcorpus retrieval field-weight learning search space must be a non-empty array",
		);
	}
	const fieldSet = new Set(index.fieldOrder ?? []);
	const seenFields = new Set<string>();
	const normalized = searchSpace.map(
		(entry): TextCorpusRetrievalFieldWeightSearchSpaceEntry => {
			if (!isRecord(entry) || !isNonEmptyString(entry.field)) {
				throw new TypeError(
					"textcorpus retrieval field-weight learning search field must be a non-empty string",
				);
			}
			if (!fieldSet.has(entry.field)) {
				throw new Error(
					`textcorpus retrieval field-weight learning references unknown field: ${entry.field}`,
				);
			}
			if (seenFields.has(entry.field)) {
				throw new Error(
					`duplicate textcorpus retrieval field-weight learning field: ${entry.field}`,
				);
			}
			...
			const sortedWeights = [...entry.weights].sort(
				(left, right) => left - right,
			);
			const weightIds = sortedWeights.map((weight) => String(weight));
			if (new Set(weightIds).size !== weightIds.length) {
				throw new Error(
					`duplicate textcorpus retrieval field-weight learning weight for field: ${entry.field}`,
				);
			}
			return {
				field: entry.field,
	...
	return normalized.sort((left, right) =>
		compareTerms(left.field, right.field),
	);
}
...
function createRetrievalFieldWeightLearningProfiles(
	...
	const visit = (fieldIndex: number): void => {
		if (fieldIndex === searchSpace.length) {
			const fields = [...selectedFields].sort((left, right) =>
				compareTerms(left.field, right.field),
			);
			profiles.push(
				createTextCorpusRetrievalFieldWeightProfile({
					profileId: retrievalFieldWeightLearningProfileId(
						profileIdPrefix,
						profiles.length + 1,
						width,
						fields,
					...
					fields: Object.fromEntries(
						fields.map((entry) => [entry.field, entry.weight]),
					),
				}),
			);
			return;
...
export function learnTextCorpusRetrievalFieldWeightProfile(
	...
	if (index.formula !== textCorpusBm25fFormula) {
		throw new TypeError(
			"textcorpus retrieval field-weight learning requires a BM25F retrieval index",
		);
	}
	if (!isRecord(options)) {
		throw new TypeError(
			"textcorpus retrieval field-weight learning options must be a record",
		);
	}
	const profileIdPrefix = options.profileIdPrefix ?? "learned-field-weights";
	if (!isNonEmptyString(profileIdPrefix)) {
		throw new TypeError(
			"textcorpus retrieval field-weight learning profile id prefix must be a non-empty string",
		);
	}
	const maxCandidateCount = Math.max(
		1,
		Math.floor(options.maxCandidateCount ?? 1024),
	);
	if (!Number.isFinite(maxCandidateCount)) {
		throw new TypeError(
			"textcorpus retrieval field-weight learning max candidate count must be finite",
		);
	}
	const searchSpace = normalizeRetrievalFieldWeightLearningSearchSpace(
		index,
		options.searchSpace,
	);
	const candidateCount =
		retrievalFieldWeightLearningCandidateCount(searchSpace);
	...
	const profiles = createRetrievalFieldWeightLearningProfiles(
		searchSpace,
		profileIdPrefix,
		candidateCount,
	);
	const calibrationOptions: TextCorpusRetrievalFieldWeightCalibrationOptions = {
		reportId:
			options.reportId ??
			`textcorpus.retrieval-field-weight-learning:${index.corpusId}`,
		...(options.optimizeMetric === undefined
			? {}
			: { optimizeMetric: options.optimizeMetric }),
		...(options.baselineCandidateId === undefined
			? {}
			: { baselineCandidateId: options.baselineCandidateId }),
		...(options.tolerance === undefined
			? {}
			: { tolerance: options.tolerance }),
		...
		...(options.relevantGradeThreshold === undefined
			? {}
			: { relevantGradeThreshold: options.relevantGradeThreshold }),
		...(options.includeBaseline === undefined
			? {}
			: { includeBaseline: options.includeBaseline }),
		...(options.searchTopK === undefined
			? {}
	...
	return calibrateTextCorpusRetrievalFieldWeightProfiles(
		index,
		queries,
		qrels,
		profiles,
...
function entryById(
	collection: TextCorpusCollectionV1,
): ReadonlyMap<string, TextCorpusEntry> {
...
export function createTextCorpusCitationWindows(
	...
	if (collection.corpusId !== result.corpusId) {
		throw new Error(
			`textcorpus citation corpus mismatch: ${collection.corpusId} != ${result.corpusId}`,
		);
	}
	...
	const windows = result.results.flatMap((queryResult) =>
		queryResult.hits.flatMap((hit): readonly TextCorpusCitationWindowV1[] => {
			...
			const text = citationTextFromTokens(
				entry,
				range.tokenStart,
				range.tokenEnd,
			);
...
function findTokenSequenceMatches(
	tokens: readonly string[],
...
export function groundTextCorpusQuote(
	...
	if (!isNonEmptyString(options.docId)) {
		throw new TypeError(
			"textcorpus quote grounding docId must be a non-empty string",
		);
	}
	if (
		!Array.isArray(options.quoteTokens) ||
		options.quoteTokens.length === 0 ||
		!options.quoteTokens.every((token) => typeof token === "string")
	) {
		throw new TypeError(
			"textcorpus quote grounding quoteTokens must be a non-empty string array",
		);
	}
	const entry = entryById(collection).get(options.docId);
	if (entry === undefined) {
		throw new Error(
			`textcorpus quote grounding docId is not in the corpus: ${options.docId}`,
		);
	}
	const tokenWindow = Math.max(0, Math.floor(options.tokenWindow ?? 0));
	const tokens = getEntryTokenTexts(entry);
	...
	const matches = starts.map((start): TextCorpusQuoteGroundingMatchV1 => {
		...
		const tokenEnd = Math.min(
			tokens.length,
			start + options.quoteTokens.length + tokenWindow,
		);
...
export function stringifyTextCorpusRetrievalIndex(
	index: TextCorpusRetrievalIndexV1,
): string {
	if (!isTextCorpusRetrievalIndexV1(index)) {
		throw new TypeError(
...
function retrievalIndexChecksum(
	index: TextCorpusRetrievalIndexV1,
): TextCorpusRetrievalIndexChecksum {
	return {
...
function canonicalTextCorpusRetrievalIndexFileSystemKey(key: string): string {
	if (
		...
		throw new TypeError(
			"textcorpus retrieval index filesystem key must be a safe relative path",
		);
	}
...
export function createTextCorpusRetrievalIndexFileSystemKey(
	...
	if (!isTextCorpusRetrievalIndexArtifactV1(artifact)) {
		throw new TypeError(
			"textcorpus retrieval index artifact checksum or shape is invalid",
		);
	}
...
export function resolveTextCorpusRetrievalIndexFileSystemPath(
	...
	if (!isNonEmptyString(root)) {
		throw new TypeError(
			"textcorpus retrieval index filesystem root must be a non-empty string",
		);
	}
...
export function parseTextCorpusRetrievalIndex(
	serialized: string,
): TextCorpusRetrievalIndexV1 {
	...
	try {
		...
		throw new SyntaxError(
			`textcorpus retrieval index JSON parse failed: ${message}`,
		);
	}
	if (!isTextCorpusRetrievalIndexV1(parsed)) {
		throw new TypeError(
			"textcorpus retrieval index JSON must satisfy TextCorpusRetrievalIndexV1",
		);
	}
	return parsed;
}
...
function assertTextCorpusRetrievalIndexStorageRefMatchesArtifact(
	...
	const expected = createTextCorpusRetrievalIndexStorageRef(
		artifact,
		ref.key,
		serialized,
	);
	if (ref.byteLength !== expected.byteLength) {
		throw new TypeError(
			"textcorpus retrieval index storage ref byteLength does not match stored artifact",
		);
	}
	if (
		...
		throw new TypeError(
			"textcorpus retrieval index storage ref checksum does not match stored artifact",
		);
	}
	if (
		...
		throw new TypeError(
			"textcorpus retrieval index storage ref metadata does not match stored artifact",
		);
	}
}
...
export async function saveTextCorpusRetrievalIndexArtifactToStore(
	...
	if (!isRecord(options) || !isNonEmptyString(options.key)) {
		throw new TypeError(
			"textcorpus retrieval index storage key must be a non-empty string",
		);
	}
	if (typeof options.writeText !== "function") {
		throw new TypeError(
			"textcorpus retrieval index storage writeText must be a function",
		);
	}
	const serialized = stringifyTextCorpusRetrievalIndexArtifact(artifact);
	await options.writeText(options.key, serialized);
	return createTextCorpusRetrievalIndexStorageRef(
		artifact,
		options.key,
		serialized,
	);
...
export async function loadTextCorpusRetrievalIndexArtifactFromStore(
	...
	if (!isRecord(options) || typeof options.readText !== "function") {
		throw new TypeError(
			"textcorpus retrieval index storage readText must be a function",
		);
	}
	const serialized = await options.readText(ref.key);
	if (typeof serialized !== "string") {
		throw new TypeError(
			"textcorpus retrieval index storage readText must return a string",
		);
	}
	const artifact = parseTextCorpusRetrievalIndexArtifact(serialized);
	assertTextCorpusRetrievalIndexStorageRefMatchesArtifact(
		ref,
		artifact,
		serialized,
	);
...
export async function saveTextCorpusRetrievalIndexArtifactToFileSystem(
	...
	if (typeof options.writeText !== "function") {
		throw new TypeError(
			"textcorpus retrieval index filesystem writeText must be a function",
		);
	}
	const key =
	...
	const resolvePath =
		options.resolvePath ?? resolveTextCorpusRetrievalIndexFileSystemPath;
	const path = resolvePath(options.root, key);
	if (!isNonEmptyString(path)) {
		throw new TypeError(
			"textcorpus retrieval index filesystem resolved path must be a non-empty string",
		);
	}
	const serialized = stringifyTextCorpusRetrievalIndexArtifact(artifact);
...
export async function loadTextCorpusRetrievalIndexArtifactFromFileSystem(
	...
	if (typeof options.readText !== "function") {
		throw new TypeError(
			"textcorpus retrieval index filesystem readText must be a function",
		);
	}
	const resolvePath =
		options.resolvePath ?? resolveTextCorpusRetrievalIndexFileSystemPath;
	const path = resolvePath(
		options.root,
		canonicalTextCorpusRetrievalIndexFileSystemKey(ref.key),
	);
	if (!isNonEmptyString(path)) {
		throw new TypeError(
			"textcorpus retrieval index filesystem resolved path must be a non-empty string",
	...
	if (typeof serialized !== "string") {
		throw new TypeError(
			"textcorpus retrieval index filesystem readText must return a string",
		);
	}
	const artifact = parseTextCorpusRetrievalIndexArtifact(serialized);
	assertTextCorpusRetrievalIndexStorageRefMatchesArtifact(
...
export function parseTextCorpusRetrievalIndexArtifact(
	...
	if (typeof serialized !== "string") {
		throw new TypeError(
			"textcorpus retrieval index artifact JSON must be a string",
		);
	}
	let parsed: unknown;
	try {
		...
		const message = error instanceof Error ? error.message : String(error);
		throw new SyntaxError(
			`textcorpus retrieval index artifact JSON parse failed: ${message}`,
		);
	}
	if (!isTextCorpusRetrievalIndexArtifactV1(parsed)) {
		throw new TypeError(
			"textcorpus retrieval index artifact JSON must satisfy checksum and shape contracts",
		);
	}
	return parsed;
}

export function isTextCorpusArtifactV1(
	value: unknown,
): value is TextCorpusArtifactV1 {
	return (
...
function isTextCorpusMetricEnvelopeMetricV1(
	...
	return (
		...
		(value.parameters === undefined ||
			isTextCorpusMetricEnvelopeMetricParameters(value.parameters))
	);
}
...
function textCorpusSelectionMetrics(
	...
	return [
		textCorpusMetric(
			"selection.document-count",
			"selection",
			selection.documentOrder.length,
			{ unit: "documents" },
		),
		textCorpusMetric(
			"selection.token-count",
			"selection",
...
function textCorpusPostingCount(index: TextCorpusRetrievalIndexV1): number {
	return Object.values(index.invertedIndex).reduce(
		(sum, postings) => sum + postings.length,
		0,
	);
}

function textCorpusRetrievalHitCount(
	result: TextCorpusRetrievalResultV1,
): number {
	return result.results.reduce(
		(sum, queryResult) => sum + queryResult.hits.length,
		0,
	);
}
...
function textCorpusMetricSetIdForArtifact(
	...
	if (isTextCorpusConcordanceResultV1(artifact))
		return `textcorpus.concordance:${artifact.corpusId}:${artifact.query}`;
	if (isTextCorpusFrequencyResultV1(artifact))
		return `textcorpus.frequency:${artifact.corpusId}`;
	if (isTextCorpusNgramResultV1(artifact))
		return `textcorpus.ngram:${artifact.corpusId}:n-${artifact.n}`;
	if (isTextCorpusCollocateResultV1(artifact))
		return `textcorpus.collocate:${artifact.corpusId}:${artifact.term}:w-${artifact.window}`;
	if (isTextCorpusCooccurrenceResultV1(artifact))
		return `textcorpus.cooccurrence:${artifact.corpusId}:w-${artifact.window}`;
	if (isTextCorpusPairwiseRelationResultV1(artifact))
		return `textcorpus.pairwise:${artifact.corpusId}`;
	if (isTextCorpusScoringResultV1(artifact))
		return `textcorpus.scoring:${artifact.corpusId}`;
	if (isTextCorpusRetrievalIndexV1(artifact))
		return `textcorpus.retrieval-index:${artifact.corpusId}:${artifact.formula}`;
	if (isTextCorpusRetrievalResultV1(artifact))
		return `textcorpus.retrieval-result:${artifact.corpusId}:${artifact.formula}`;
	if (isTextCorpusRetrievalEvaluationResultV1(artifact))
		return `textcorpus.retrieval-evaluation:${artifact.corpusId}:${artifact.formula}:k-${artifact.k}`;
	if (isTextCorpusRetrievalCalibrationReportV1(artifact))
		return `textcorpus.retrieval-calibration:${artifact.corpusId}:${artifact.formula}:k-${artifact.k}:${artifact.optimizeMetric}`;
	if (isTextCorpusCitationWindowSetV1(artifact))
		return `textcorpus.citation-windows:${artifact.corpusId}`;
...
export function exportTextCorpusMetricEnvelopePayloadV1(
	...
	if (!isTextCorpusArtifactV1(artifact)) {
		throw new TypeError(
			"textcorpus metric envelope payload requires a known TextCorpus artifact",
		);
	}
	const source = isTextCorpusRetrievalIndexArtifactV1(artifact)
		? artifact.index
		: artifact;
	const metricSetId =
		options.metricSetId ?? textCorpusMetricSetIdForArtifact(source);
	...
	if (isTextCorpusConcordanceResultV1(source)) {
		...
		metrics.push(
			...
			textCorpusMetric(
				...
				"frequency",
				source.rows.reduce((sum, row) => sum + row.count, 0),
				{ unit: "tokens" },
			),
		);
	} else if (isTextCorpusNgramResultV1(source)) {
		metrics.push(
			textCorpusMetric("ngram.row-count", "ngram", source.rows.length, {
				unit: "ngrams",
				parameters: { n: source.n },
			}),
			textCorpusMetric(
				"ngram.total-count",
				"ngram",
				source.rows.reduce((sum, row) => sum + row.count, 0),
				{ unit: "ngrams", parameters: { n: source.n } },
			),
		);
	} else if (isTextCorpusCollocateResultV1(source)) {
		metrics.push(
			textCorpusMetric("collocate.row-count", "collocate", source.rows.length, {
				unit: "rows",
				parameters: { term: source.term, window: source.window },
			}),
			textCorpusMetric(
				"collocate.total-count",
				"collocate",
				source.rows.reduce((sum, row) => sum + row.count, 0),
				{
					unit: "cooccurrences",
		...
		metrics.push(
			textCorpusMetric(
				"cooccurrence.row-count",
				"cooccurrence",
				source.rows.length,
				{ unit: "pairs", parameters: { window: source.window } },
			),
			textCorpusMetric(
				"cooccurrence.total-count",
				"cooccurrence",
				source.rows.reduce((sum, row) => sum + row.count, 0),
				{ unit: "pairs", parameters: { window: source.window } },
			),
		...
		metrics.push(
			textCorpusMetric("pairwise.row-count", "pairwise", source.rows.length, {
				unit: "relations",
			}),
			textCorpusMetric(
				"pairwise.exact-duplicate-count",
				"pairwise",
				textCorpusRelationCount(source, "exact-duplicate"),
				{ unit: "relations" },
			),
			textCorpusMetric(
				"pairwise.near-duplicate-count",
				"pairwise",
				textCorpusRelationCount(source, "near-duplicate"),
				{ unit: "relations" },
			),
			textCorpusMetric(
				"pairwise.shared-reuse-count",
				"pairwise",
				textCorpusRelationCount(source, "shared-reuse"),
				{ unit: "relations" },
			),
		);
		...
		metrics.push(
			textCorpusMetric(
				"scoring.document-count",
				"scoring",
				source.documentOrder.length,
				{ unit: "documents" },
			),
			textCorpusMetric(
				"scoring.term-count",
				"scoring",
				source.termOrder.length,
				{ unit: "terms" },
			),
			textCorpusMetric(
				"scoring.query-count",
		...
		metrics.push(
			textCorpusMetric(
				"retrieval-index.document-count",
				"retrieval",
				source.documentOrder.length,
				{ unit: "documents", parameters: { formula: source.formula } },
			),
			textCorpusMetric(
				"retrieval-index.term-count",
				"retrieval",
				source.termOrder.length,
				{ unit: "terms", parameters: { formula: source.formula } },
			),
			textCorpusMetric(
				"retrieval-index.average-document-length",
				"retrieval",
				source.averageDocumentLength,
				{ unit: "tokens", parameters: { formula: source.formula } },
			),
			textCorpusMetric(
				"retrieval-index.posting-count",
				"retrieval",
				textCorpusPostingCount(source),
				{ unit: "postings", parameters: { formula: source.formula } },
			),
		);
	} else if (isTextCorpusRetrievalIndexStorageRefV1(source)) {
		metrics.push(
			textCorpusMetric(
				"retrieval-index-storage-ref.document-count",
				"retrieval",
				source.documentCount,
				{ unit: "documents", parameters: { formula: source.formula } },
			),
			textCorpusMetric(
				"retrieval-index-storage-ref.term-count",
				"retrieval",
				source.termCount,
				{ unit: "terms", parameters: { formula: source.formula } },
			),
			textCorpusMetric(
				"retrieval-index-storage-ref.field-count",
				"retrieval",
				source.fieldCount,
				{ unit: "fields", parameters: { formula: source.formula } },
			),
			textCorpusMetric(
				"retrieval-index-storage-ref.byte-length",
				"retrieval",
				source.byteLength,
				{ unit: "utf8-bytes", parameters: { formula: source.formula } },
			),
		);
	} else if (isTextCorpusRetrievalResultV1(source)) {
		metrics.push(
			textCorpusMetric(
				"retrieval-result.query-count",
				"retrieval",
				source.results.length,
				{ unit: "queries", parameters: { formula: source.formula } },
			),
			textCorpusMetric(
				"retrieval-result.hit-count",
				"retrieval",
				textCorpusRetrievalHitCount(source),
				{ unit: "hits", parameters: { formula: source.formula } },
			),
		);
	} else if (isTextCorpusRetrievalEvaluationResultV1(source)) {
		metrics.push(
			textCorpusMetric(
				"retrieval-evaluation.query-count",
				...
				source.queries.length,
				{
					...
					parameters: {
						k: source.k,
						relevantGradeThreshold: source.relevantGradeThreshold,
					},
				},
			),
			textCorpusMetric(
				"retrieval-evaluation.precision-at-k",
				"retrieval-evaluation",
				source.summary.precisionAtK,
				{ unit: "ratio", parameters: { k: source.k } },
			),
			textCorpusMetric(
				"retrieval-evaluation.recall-at-k",
				"retrieval-evaluation",
				source.summary.recallAtK,
				{ unit: "ratio", parameters: { k: source.k } },
			),
			textCorpusMetric(
			...
			textCorpusMetric(
				"retrieval-evaluation.ndcg-at-k",
				"retrieval-evaluation",
				source.summary.ndcgAtK,
				{ unit: "ratio", parameters: { k: source.k } },
			),
		);
	} else if (isTextCorpusRetrievalCalibrationReportV1(source)) {
		const selectedCandidate = source.candidates.find(
			(candidate) => candidate.selected,
		);
		metrics.push(
			textCorpusMetric(
				"retrieval-calibration.candidate-count",
				"retrieval-calibration",
				source.candidates.length,
				{
					unit: "candidates",
					parameters: { k: source.k, optimizeMetric: source.optimizeMetric },
				},
			),
			textCorpusMetric(
				"retrieval-calibration.selected-candidate-id",
				"retrieval-calibration",
				source.selectedCandidateId,
				{ parameters: { optimizeMetric: source.optimizeMetric } },
			),
			textCorpusMetric(
				"retrieval-calibration.selected-metric-score",
				"retrieval-calibration",
				selectedCandidate?.metricScore ?? 0,
				{
					unit: "ratio",
					parameters: { optimizeMetric: source.optimizeMetric },
				},
			...
			textCorpusMetric(
				"retrieval-calibration.within-tolerance-count",
				"retrieval-calibration",
				source.candidates.filter(
					(candidate) => candidate.withinToleranceOfSelected,
				).length,
				{ unit: "candidates", parameters: { tolerance: source.tolerance } },
			),
		);
	} else if (isTextCorpusCitationWindowSetV1(source)) {
		metrics.push(
			textCorpusMetric(
				"citation-window.window-count",
				"citation-window",
				source.windows.length,
				{ unit: "windows" },
			),
			textCorpusMetric(
				"citation-window.loss-count",
				"citation-window",
				source.windows.reduce((sum, window) => sum + window.loss.length, 0),
				{ unit: "loss-records" },
			),
		);
		...
		metrics.push(
			textCorpusMetric(
				"quote-grounding.match-count",
				"quote-grounding",
				source.matches.length,
				{ unit: "matches" },
			),
			textCorpusMetric(
				"quote-grounding.status",
				"quote-grounding",
				source.status,
			),
		);
	...
	if (!isTextCorpusMetricEnvelopePayloadV1(payload)) {
		throw new TypeError(
			"textcorpus metric envelope payload could not be produced",
		);
	}
	return payload;
}

export function stringifyTextCorpusArtifact(
	artifact: TextCorpusArtifactV1,
): string {
	if (!isTextCorpusArtifactV1(artifact)) {
		throw new TypeError(
			"textcorpus artifact must satisfy a known TextCorpus artifact contract",
		);
	}
	return `${JSON.stringify(artifact, null, 2)}\n`;
}

export function parseTextCorpusArtifact(
	serialized: string,
): TextCorpusArtifactV1 {
	if (typeof serialized !== "string") {
	...
	if (!isTextCorpusArtifactV1(parsed)) {
		throw new TypeError(
			"textcorpus artifact JSON must satisfy a known TextCorpus artifact contract",
		);
	}
	return parsed;
}
