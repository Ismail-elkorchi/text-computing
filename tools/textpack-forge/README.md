...
The current active graph is source-backed foundation only:
...
- source policy specs under `tools/textpack-forge/source-policies/*`
  describe the metadata-only source universe before language ingestion begins.
...
and transitional packs are excluded from the active package graph.
