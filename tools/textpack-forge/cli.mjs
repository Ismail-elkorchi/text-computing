		...
		diagnostics: [
			{
				...
				severity: "info",
				message: `Unicode/CLDR-backed boundary profile for ${config.scopeLabel}; dictionary tokenization and sentence-abbreviation tailoring are out of scope.`,
				metadata: {
					likelySubtag,
					sourceIds: resourceSpec.sourceIds,
				},
...
function transformWikidataMainArtifact(resourceSpec, inputs) {
	...
	const summary = {
		...
		warnings: [
			...
			"The full Wikidata dump is artifact-backed and must be fetched explicitly; it is not vendored in the npm package.",
		],
	};
	const canonicalQuality = {
		schemaVersion: "1",
		...
		diagnostics: [
			{
				...
				task: "kb.artifact",
				severity: "info",
				message: `Wikidata main entity data for ${config.languageName} KB consumers is exposed as an explicit full artifact descriptor, not as an npm-shipped raw dump.`,
				metadata: {
					artifactId: config.artifactId,
		...
		diagnostics: [
			{
				...
				severity: "info",
				message:
					`Tatoeba ${config.languageName} sentence data is exposed as an explicit full artifact descriptor, not as npm-shipped raw corpus text.`,
				metadata: {
					artifactId: corpusArtifact.artifactId,
					artifactPolicy: "fetch-explicit",
				},
		...
		diagnostics: [
			{
				...
				severity: "info",
				message:
					`Tatoeba ${config.languageName} parallel alignments are exposed as explicit full artifact descriptors, not as npm-shipped raw parallel text.`,
				metadata: {
					artifactPolicy: "fetch-explicit",
					artifactIds: linkArtifacts.map((artifact) => artifact.artifactId),
				},
...
function transformFrenchLexique383(resourceSpec, inputs) {
	...
	const quality = {
		schemaVersion: "1",
		sourceId: "source:fr:lexique-383",
		pipelineId: resourceSpec.pipelineId,
		pipelineVersion: resourceSpec.pipelineVersion,
		release: "3.83",
		entryCount: entryRows.length,
	...
	const canonicalSearchProfile = {
		...
		charFilters: [
			{
				componentId: "french-apostrophe-policy",
				type: "character-policy",
				mode: "preserve-elision-apostrophes",
				options: {
					characters: ["'", "’"],
				},
			},
...
function knownGaps(packSpec, manifest) {
	...
	if (packSpec.packClass === "language-composite") {
		gaps.push(
			"generated language recipe composite; see declared scope for excluded isolated sources and explicit artifact-fetch limitations",
		);
...
function publishabilityFor(spec, manifest, context) {
	...
	if (
		...
		reasons.push(
			"developer-facing language composites require feature-complete support",
		);
	}
	if (
		requested &&
...
function packSupportsReadinessSlot(pack, slot) {
	...
	return pack.capabilitySlots.some(
		(capabilitySlot) =>
			...
			!["unsupported", "planned", "not-applicable"].includes(
				capabilitySlot.status,
			),
	);
}
...
function languageSlotSourceRequirementBlockers({
	...
	if (exactPack === undefined) {
		return requirements.map((requirement) => {
			...
			return `${requirement.label}; expected one of ${sourceIds.join(", ")} before full ${languageTag} ${slot} readiness can be promoted.`;
		});
	}
	return requirements
		.filter(
...
function languageSlotBlockers({
	...
	if (exactPack === undefined) {
		...
		if (candidatePacks.length > 0) {
			blockers.push(
				"Candidate source-backed packs exist, but the required slot package has not been promoted.",
			);
		}
...
function languageCompositeReadinessMarkdown(readiness) {
	const lines = [
		...
		"",
		"This report is generated from the active forge graph. It gates developer-facing language composites: `textpack-en`, `textpack-ar`, and `textpack-fr` stay non-publishable until every required slot is composite-ready.",
		"",
		"Candidate packs are informational only. A candidate does not satisfy a slot until the exact required package is generated, audited, evaluated, publishable, and declares production coverage for that slot.",
		"",
		"## Summary",
		"",
