...
function renderPackageReleaseGates([gates]) {
  return [
    ...
    "",
    `Phase: \`${gates.phaseCompletionEvidence.phase}\`.`,
    "",
    `Claim boundary: ${gates.phaseCompletionEvidence.claimBoundary}`,
    "",
    "Evidence:",
    "",
    mdList(gates.phaseCompletionEvidence.evidenceRefs),
    "",
    "Commands:",
    "",
    mdList(gates.phaseCompletionEvidence.commands.map((command) => `\`${command}\``)),
    "",
    ...
    table(
      ...
      gates.packages.flatMap((entry) =>
        (entry.blockerClassifications ?? []).map((classification) => [
          `\`${entry.packageName}\``,
          `\`${classification.maturity}\``,
          classification.blocker,
          classification.decision,
          list(classification.evidenceRefs),
        ]),
      ),
    ),
    "",
