...
function assertDeepEqualJson(actual, expected, label) {
	...
	expect(actualJson === expectedJson, `${label} must match pack.manifest.json.`);
}
...
expect(packDirs.length >= 10, "Final textpack reference package coverage is incomplete.");
...
for (const packDir of packDirs) {
	...
	expect(Array.isArray(packageJson.files), `${packageJson.name} must declare publish files.`);
	expect(packageJson.files.includes("dist"), `${packageJson.name} files must include dist.`);
	expect(
	...
	expect(!packageJson.files.includes("test"), `${packageJson.name} files must not publish test.`);
	expect(packageJson.exports?.["."]?.import === "./dist/index.js", `${packageJson.name} root import must use dist/index.js.`);
	expect(packageJson.exports?.["."]?.types === "./dist/index.d.ts", `${packageJson.name} root types must use dist/index.d.ts.`);
	expect(
	...
	const actualKinds = new Set(manifest.resources.map((resource) => resource.kind));
	expect(
		JSON.stringify(sorted(declaredKinds)) === JSON.stringify(sorted(actualKinds)),
		`${manifestPath} kind must exactly match resource descriptor kinds.`,
	);

	const resourceIds = new Set();
	for (const resource of manifest.resources) {
		expect(!resourceIds.has(resource.id), `${manifestPath} duplicates resource id ${resource.id}.`);
		resourceIds.add(resource.id);
		if (resource.path !== undefined) {
			assertPackageRelativePath(resource.path, `${manifest.packageName} resource path ${resource.path}`);
			const relativePath = `packages/${packDir}/${resource.path}`;
			expect(await fileExists(relativePath), `${manifest.packageName} missing resource ${resource.path}.`);
			const content = await readFile(path.join(ROOT, relativePath), "utf8");
			const nonEmptyLineCount = content
				.split(/\r?\n/u)
				.map((line) => line.trim())
				.filter((line) => line.length > 0).length;
			expect(nonEmptyLineCount > 0, `${manifest.packageName} resource ${resource.path} must not be empty.`);
		}
	}

	const builtPack = await maybeImportBuiltPack(packDir);
	if (builtPack !== undefined) {
		expect(builtPack.manifest !== undefined, `${packageJson.name} must export manifest.`);
		expect(builtPack.resources !== undefined, `${packageJson.name} must export resources.`);
		expect(builtPack.default?.manifest === builtPack.manifest, `${packageJson.name} default manifest must reuse manifest export.`);
		expect(builtPack.default?.resources === builtPack.resources, `${packageJson.name} default resources must reuse resources export.`);
		assertDeepEqualJson(builtPack.manifest, manifest, `${packageJson.name} built manifest`);
		const resourceKeys = Object.keys(builtPack.resources);
		expect(
			JSON.stringify(sorted(resourceKeys)) === JSON.stringify(sorted(resourceIds)),
